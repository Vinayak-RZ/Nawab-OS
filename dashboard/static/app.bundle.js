var Ue=(e,S=document)=>S.querySelector(e),qe=(e,S=document)=>[...S.querySelectorAll(e)];function ie(e){e.$=Ue,e.$$=qe}function Je(e){let S=document.createElement("div");return S.textContent=e??"",S.innerHTML}function Ke(e){return"$"+Number(e||0).toLocaleString(void 0,{maximumFractionDigits:0})}function ze(e){return e?new Date(typeof e=="number"&&e<1e12?e*1e3:e).toLocaleString():""}function Ye(e){return new Promise(S=>setTimeout(S,e))}function le(e){e.esc=Je,e.fmtMoney=Ke,e.fmtTime=ze,e.sleep=Ye}function Qe(e,S){try{let k=localStorage.getItem(e);return k?JSON.parse(k):S}catch(k){return console.warn(`[storage] corrupt ${e}, resetting`,k),localStorage.removeItem(e),S}}function de(e){e.readJsonStorage=Qe}var Xe="Nawab OS",Ze=[{id:"pulse",label:"Pulse",role:"aggregator",tool_count:0,brief:"Operating pulse across parallel projects"},{id:"outreach",label:"Outreach",role:"outreach",tool_count:0,brief:"Outreach drafts and CRM pipeline"},{id:"leads",label:"Leads",role:"leads",tool_count:0,brief:"Lead lists and contact priorities"},{id:"market",label:"Market intel",role:"research",tool_count:0,brief:"Industry and competitor intelligence"},{id:"vault",label:"Vault",role:"knowledge",tool_count:0,brief:"Knowledge vault librarian"}],xe=[{id:"auto",label:"Auto",hint:"Agent picks retrieval"},{id:"hybrid",label:"Hybrid RAG",hint:"Dense + BM25 fusion"},{id:"graphrag",label:"GraphRAG",hint:"Knowledge graph communities"},{id:"vault",label:"Vault",hint:"World knowledge vault"},{id:"documents",label:"Documents",hint:"Ingested document store"}],et={dashboard:"Control center",chat:"Ask agent",agents:"Agent fleet",world:"Worlds",approvals:"Approvals",crm:"CRM & pipeline",outreach:"Outreach",goals:"Goals & tasks",memory:"Memory",documents:"Documents",history:"History",tools:"Tools",activity:"Activity",settings:"Settings"},tt=["prospect","contacted","replied","meeting","won","lost","nurture"],at=["prospect","contacted","responded","meeting_set","closed","dead"],st=["#f75440","#00666b","#03904a","#051f13","#5a403c","#8f706b","#e3beb8"],nt=15,ot=30,rt=5e3,it=3e4,lt=3e4,dt={aggregator:{label:"Aggregator",cls:"agent-role--aggregator",avatar:"agent-avatar--aggregator"},outreach:{label:"Outreach",cls:"agent-role--outreach",avatar:"agent-avatar--outreach"},leads:{label:"Leads",cls:"agent-role--leads",avatar:"agent-avatar--leads"},research:{label:"Intel",cls:"agent-role--research",avatar:"agent-avatar--research"},knowledge:{label:"Vault",cls:"agent-role--vault",avatar:"agent-avatar--knowledge"}},ct={supervisor:"SV",pulse:"PL",outreach:"OR",leads:"LD",market:"MK",vault:"VL"},ut={root:{label:"Main",cls:"world-kind--root"},project:{label:"Startup",cls:"world-kind--project"},startup:{label:"Startup",cls:"world-kind--project"},technical:{label:"Technical",cls:"world-kind--research"},idea:{label:"Idea",cls:"world-kind--idea"},research:{label:"Research",cls:"world-kind--research"}};function ce(e){Object.assign(e,{APP_NAME:Xe,DEFAULT_SPECIALISTS:Ze,RAG_MODES:xe,TITLES:et,CRM_STATUSES:tt,COMPANY_STATUSES:at,CHART_COLORS:st,MSG_READ_INITIAL_LINES:nt,MSG_READ_EXPAND_LINES:ot,LIVE_POLL_MS:rt,LIVE_POLL_HIDDEN_MS:it,REFRESH_MS:lt,AGENT_ROLES:dt,AGENT_INITIALS:ct,WORLD_KINDS:ut})}function ue(e){async function S(b,a,p="POST"){let u=await fetch("/api"+b,{method:p,body:a,credentials:"same-origin"}),h=await u.json().catch(()=>({}));if(u.status===401&&h.pin_required)throw e.showPinGate(),new Error("Enter your PIN to continue");if(!u.ok)throw new Error(h.error||u.statusText);return h}async function k(b,a={}){let p=new AbortController,u=a.timeoutMs??3e4,h=setTimeout(()=>p.abort(),u),{timeoutMs:n,headers:t,signal:s,...l}=a;try{let i=await fetch("/api"+b,{...l,credentials:"same-origin",headers:{"Content-Type":"application/json",...t||{}},signal:s||p.signal}),g=await i.json().catch(()=>({}));if(i.status===401&&g.pin_required)throw e.showPinGate(),new Error("Enter your PIN to continue");if(!i.ok)throw new Error(g.error||i.statusText);return g}catch(i){throw i.name==="AbortError"?new Error("Request timed out \u2014 is the server running?"):i}finally{clearTimeout(h)}}e.api=k,e.apiUpload=S}function pe(e){function S(){let k=localStorage.getItem("fos_selected_specialist");if(k!==null)return k;let b=localStorage.getItem("fos_selected_agent");return b&&b!=="supervisor"?b:""}e.state={live:{},selectedSpecialist:S(),ragMode:localStorage.getItem("fos_rag_mode")||"auto",activeWorldId:localStorage.getItem("fos_active_world")||"root",agentsTab:localStorage.getItem("fos_agents_tab")||"runs",expandedRunId:null,ui:{worldCreateOpen:!1,crmFormOpen:!1,goalsFormOpen:!1,reminderFormOpen:!1,vaultFacet:null,vaultDocForm:null,vaultDocEdit:null},_worldTemplates:null,_operations:{},_chatAttachments:[]},e.state._syncingLinkIds=new Set,e.currentView="dashboard",e.chatHistory=e.readJsonStorage("fos_chat",[]),e.historyTab=localStorage.getItem("fos_history_tab")||"conversations",e.documentsEditMode=!1,e.mdEditorState={mode:null,artifactId:null,worldId:null,docId:null,editMode:!1},e.livePollTimer=null,e._runtimePollTick=0,e.whatsappPollTimer=null,e.memoryGraphTab="graph",e.worldGraphTab="hierarchy",e.lastLiveActive=!1,e.viewDataLoadGen=0,e.vaultLoadGen=0,e.graphDrawCache={},e.actionBusyDepth=0,e.actionBusyButton=null,e.refreshTimer=null,e.loadSelectedSpecialist=S}function me(e){function S(){let l=e.state.config||{};return l.my_name?`${l.my_name}'s ${e.APP_NAME}`:e.APP_NAME}function k(){return e.state.activeWorldId||e.$("#world-select")?.value||"root"}function b(){let l=e.state.worlds||e.state._worldFull?.worlds||{},i=e.currentWorldId();return i==="root"?l.root?.name||"Main world":(l.children||[]).find(T=>T.id===i)?.name||i}function a(l){e.state.activeWorldId=l||"root",localStorage.setItem("fos_active_world",e.state.activeWorldId),e.populateWorldSelect(),e.updateWorldContextChrome()}function p(){let l=e.$("#world-select");if(!l)return;let i=e.state.activeWorldId||"root";[...l.options].some(g=>g.value===i)&&(l.value=i)}function u(){let l=e.activeWorldLabel();document.querySelectorAll("[data-active-world-label]").forEach(i=>{i.textContent=l}),e.syncWorldSelectValue(),e.currentView==="world"&&e.patchWorldTreeNav()}function h(){let l=e.$("#specialist-select-agents")?.value??e.state.selectedSpecialist??"";return l==="auto"?"":l||""}function n(){return e.$("#rag-mode-select")?.value||e.state.ragMode||"auto"}function t(){return!!e.currentSpecialistId()}function s(){let l=e.$("#world-select");if(!l)return;let i=e.state.worlds||e.state._worldFull?.worlds||{},g=i.root,T=i.children||[],A=T.map(y=>`<option value="${e.esc(y.id)}">${e.esc(y.name)} \xB7 ${e.esc(y.kind||"project")}</option>`).join("");l.innerHTML=`
      ...
      ${g.length?`<label class="live-phase-select"><span class="caption-uppercase">Step</span>
        <select class="world-select" id="${t}-step" aria-label="Current step">${T}</select></label>`:""}
      ...
      </div>`}).join("")}async function b(p,u,h={}){let n=p;e.state._operations[n]={id:n,title:u,detail:"Scanning repository\u2026",progress:0,status:"running"},h.linkId!=null&&e.state._syncingLinkIds.add(String(h.linkId)),e.renderOpsStack(),h.worldId&&e.currentView==="world"&&e.render();try{for(;;){let t=await e.api(`/sync-jobs/${encodeURIComponent(p)}/batch`,{method:"POST",body:JSON.stringify({batch_size:8}),timeoutMs:18e4}),s=e.state._operations[n];if(s&&(s.progress=t.progress||0,s.detail=t.message||`${t.imported||0} files imported`,s.status=t.status==="failed"?"error":t.done?"done":"running"),e.renderOpsStack(),t.done)break}}catch(t){let s=e.state._operations[n];throw s&&(s.status="error",s.detail=t.message||"Sync failed",s.finishedAt=Date.now()),e.renderOpsStack(),t}finally{let t=e.state._operations[n];t&&!t.finishedAt&&(t.finishedAt=Date.now()),h.linkId!=null&&e.state._syncingLinkIds.delete(String(h.linkId)),e.renderOpsStack();try{await e.refresh(),h.worldId&&await e.reloadVault(h.worldId,{force:!0}),e.currentView==="world"?e.patchWorldPanels():e.currentView==="agents"&&e.patchAgentsVaultPanel(),e.updateBadges()}catch{}setTimeout(()=>{delete e.state._operations[n],e.renderOpsStack()},8e3)}}async function a(p){let u=await e.api(`/worlds/${encodeURIComponent(p)}/sync-jobs`).catch(()=>({jobs:[]}));for(let h of u.jobs||[])!h?.id||e.state._operations[h.id]||e.runGithubSyncJob(h.id,`Syncing ${h.full_name}`,{worldId:p,linkId:h.link_id}).catch(console.error)}e.isLinkSyncing=S,e.renderOpsStack=k,e.runGithubSyncJob=b,e.resumeActiveSyncJobs=a}function ve(e){function S(){e.mdEditorState={mode:null,artifactId:null,worldId:null,docId:null,editMode:!1},e.$("#md-dialog-source").hidden=!0,e.$("#md-dialog-preview").hidden=!1,e.$("#md-dialog-save").hidden=!0,e.$("#md-dialog-mode").hidden=!1,e.$("#md-dialog-mode").textContent="Edit"}async function k(p,u,h){let n=e.$("#md-editor-dialog");if(!(!n||!p||!u)){e.mdEditorState={mode:"vault",artifactId:null,worldId:p,docId:u,editMode:!1},e.$("#md-dialog-title").textContent=h||"Document",e.$("#md-dialog-preview").hidden=!1,e.$("#md-dialog-source").hidden=!0,e.$("#md-dialog-save").hidden=!0,e.$("#md-dialog-mode").hidden=!1,e.$("#md-dialog-mode").textContent="Edit",e.$("#md-dialog-preview").innerHTML="<p class='body-md muted'>Loading\u2026</p>",n.showModal();try{let s=(await e.api(`/worlds/${encodeURIComponent(p)}/vault/documents/${encodeURIComponent(u)}/content`,{timeoutMs:2e4})).content||"";e.$("#md-dialog-source").value=s;let l=e.$("#md-dialog-preview");await window.FOSMarkdown?.renderInto?.(l,s)}catch(t){e.$("#md-dialog-preview").innerHTML=`<p class="body-md" style="color:var(--color-warn)">${e.esc(t.message||"Could not load document")}</p>`}}}async function b(){let p=e.$("#md-dialog-source")?.value??"";if(e.mdEditorState.mode==="vault"&&e.mdEditorState.worldId&&e.mdEditorState.docId){await e.api(`/worlds/${encodeURIComponent(e.mdEditorState.worldId)}/vault/documents/${encodeURIComponent(e.mdEditorState.docId)}`,{method:"PATCH",body:JSON.stringify({content:p}),timeoutMs:15e3});let h=e.$("#md-dialog-preview");await window.FOSMarkdown?.renderInto?.(h,p),e.mdEditorState.editMode=!1,e.$("#md-dialog-source").hidden=!0,e.$("#md-dialog-preview").hidden=!1,e.$("#md-dialog-save").hidden=!0,e.$("#md-dialog-mode").textContent="Edit";return}if(!e.mdEditorState.artifactId)return;await e.api(`/artifacts/${e.mdEditorState.artifactId}/content`,{method:"PUT",body:JSON.stringify({content:p}),timeoutMs:15e3});let u=e.$("#md-dialog-preview");await window.FOSMarkdown?.renderInto?.(u,p),e.mdEditorState.editMode=!1,e.$("#md-dialog-source").hidden=!0,e.$("#md-dialog-preview").hidden=!1,e.$("#md-dialog-save").hidden=!0,e.$("#md-dialog-mode").textContent="Edit"}function a(){e.$("#md-dialog-close")?.addEventListener("click",()=>{e.$("#md-editor-dialog")?.close(),e.resetMdEditorDialog()}),e.$("#md-dialog-mode")?.addEventListener("click",async()=>{if(e.mdEditorState.mode!=="vault"&&!e.mdEditorState.artifactId)return;e.mdEditorState.editMode=!e.mdEditorState.editMode;let p=e.$("#md-dialog-source"),u=e.$("#md-dialog-preview");if(e.mdEditorState.editMode)p.hidden=!1,u.hidden=!0,e.$("#md-dialog-save").hidden=!1,e.$("#md-dialog-mode").textContent="Preview";else{let h=p?.value??"";await window.FOSMarkdown?.renderInto?.(u,h),p.hidden=!0,u.hidden=!1,e.$("#md-dialog-save").hidden=!0,e.$("#md-dialog-mode").textContent="Edit"}}),e.$("#md-dialog-save")?.addEventListener("click",()=>e.saveMdEditor().catch(p=>alert(p.message)))}e.resetMdEditorDialog=S,e.openVaultDocViewer=k,e.saveMdEditor=b,e.initMdEditorDialog=a}function ye(e){function S(){let n=e.state._nudges||[];return n.length?`<section class="driver-card span-12 up-next-panel">
      ...
      </section>`}function u(n){if(e.state.ui||(e.state.ui={}),n==="create-world"){e.state.ui.worldCreateOpen=!0,e.currentView==="world"?(e.render(),requestAnimationFrame(()=>document.getElementById("world-create-panel")?.scrollIntoView({behavior:"smooth",block:"nearest"}))):(e.goView("world"),e.state._scrollWorldCreate=!0);return}if(n==="add-contact"){e.state.ui.crmFormOpen=!0,e.currentView==="crm"?e.render():e.goView("crm");return}if(n==="add-goal"){e.state.ui.goalsFormOpen=!0,e.currentView==="goals"?e.render():e.goView("goals");return}if(n==="add-reminder"){e.state.ui.reminderFormOpen=!0,e.currentView==="goals"?e.render():e.goView("goals");return}n==="settings"&&e.goView("settings"),n==="approvals"&&e.goView("approvals")}function h(){let n=e.state.snapshot||{},t=n.crm||{},s=e.state.finance||{},l=e.state.usage||{},i=e.state.about||{},g=e.state.config||{},T=n.approvals_pending||0,A=s.set?`<span class="pill ${s.status==="healthy"?"ok":s.status==="warning"?"warn":"info"}">${e.esc(s.status)}</span>`:"",v=s.set?s.runway||(s.runway_months!=null?s.runway_months+" mo":"\u2014"):null,y=(e.state.goals||[]).slice(0,5).map(q=>`<li>${e.esc(q.title)}</li>`).join("")||"<li class='muted'>No active goals \u2014 add one in Goals or use Direct controls.</li>",L=T>0?`<div class="spec-cell race-position-cell"><dt>Approvals</dt><dd>${T}</dd></div>`:'<div class="spec-cell"><dt>Approvals</dt><dd>0</dd></div>',W=e.state.live||{},N=e.state._agents||{};return`<div class="dashboard-grid">
        ${e.renderUpNext()}
      ...
      </div>`}e.renderUpNext=S,e.handleNudgeAction=k,e.chartPanelNote=b,e.drawDashboardCharts=a,e.renderOperatorPanel=p,e.openOperatorAction=u,e.renderDashboard=h}function we(e){function S(){return localStorage.getItem("fos_chat_session")||""}function k(C){C?localStorage.setItem("fos_chat_session",C):localStorage.removeItem("fos_chat_session")}function b(C){C?.session_id&&e.setChatSessionId(C.session_id)}async function a(){let C=e.chatSessionId();if(C)try{let r=await e.api(`/history/sessions/${C}`);r?.messages?.length&&(e.chatHistory=r.messages.map(c=>({role:c.role==="assistant"?"agent":c.role,text:c.content})),localStorage.setItem("fos_chat",JSON.stringify(e.chatHistory)))}catch{}}function p(C={}){let r={world_id:e.currentWorldId(),rag_mode:e.currentRagMode(),session_id:e.chatSessionId()||void 0,specialist:e.currentSpecialistId()||void 0,...C},c=(e.state._chatAttachments||[]).filter(w=>w?.doc_id);return c.length&&(r.attachments=c.map(w=>({type:"vault",doc_id:w.doc_id,title:w.title,path:w.path}))),r}function u(C){if(C.pending)return`<div class="msg-pending"><span class="live-pulse" aria-hidden="true"></span> ${e.esc(C.pendingLabel||"Agent working\u2026")}</div>`;let r=C.text||"";if(C.role==="agent"||C.role==="assistant"){let c=window.FOSMarkdown?.render?.(r)||e.esc(r),w=(C.artifacts||[]).map(I=>`<button type="button" class="button-outline-on-dark button-sm md-artifact-btn" data-md-artifact="${I.id}">${e.esc(I.title||I.kind||"Document")}</button>`).join("");return`<div class="msg-md">${c}</div>${w?`<div class="msg-artifacts">${w}</div>`:""}`}return`<div class="msg-plain">${e.esc(r)}</div>`}function h(C,r){return`msg:${C}:${e.chatSessionId()||"default"}:${r}`}function n(C){return C<=0?e.MSG_READ_INITIAL_LINES:C===1?e.MSG_READ_INITIAL_LINES+e.MSG_READ_EXPAND_LINES:1/0}function t(C){let r=C||document.getElementById("content");r&&(e.state._msgExpand||(e.state._msgExpand={}),r.querySelectorAll(".msg-read-more-host").forEach(c=>{let w=c.querySelector(":scope > .msg-md, :scope > .msg-plain"),I=c.querySelector(".msg-read-more");if(!w||!I)return;let M=c.dataset.msgScope||"chat",F=c.dataset.msgIndex??"0",U=e.msgExpandKey(M,F),m=e.state._msgExpand[U]||0,_=parseFloat(getComputedStyle(w).lineHeight)||21,P=Math.max(1,Math.round(w.scrollHeight/_)),E=e.msgReadLineLimit(m);if(I.dataset.msgReadMore=U,E>=P||m>=2){w.classList.remove("msg-body--clamped"),w.style.maxHeight="",I.hidden=!0;return}w.classList.add("msg-body--clamped"),w.style.maxHeight=`${E*_}px`,I.hidden=!1,I.textContent="Read more"}))}function s(C){return C?.length?`<div class="msg-artifacts">${C.map(r=>`<button type="button" class="button-outline-on-dark button-sm md-artifact-btn" data-md-artifact="${r.id}">${e.esc(r.title||r.kind||"File")}</button>`).join("")}</div>`:""}async function l(){let C=e.currentWorldId(),r=C&&C!=="root"?`?world_id=${encodeURIComponent(C)}`:"";try{let c=await e.api(`/history${r}`,{timeoutMs:15e3});e.state._chatSessions=c.sessions||[]}catch{e.state._chatSessions=e.state._chatSessions||[]}}function i(){let C=e.state._chatSessions||[],r=e.chatSessionId();return`<section class="chat-sessions-strip driver-card">
    ...
    </section>`}async function g(C){e.openDocumentsWorkspace(C)}function T(){let C=e.state._chatAttachments||[];return C.length?`<div class="chat-attachments">${C.map((r,c)=>`<span class="chat-attachment-chip">
      ...
      </span>`).join("")}</div>`:""}async function A(){let C=e.currentWorldId();if(!C||C==="root"){alert("Select a project world (not Main) to attach vault documents.");return}await e.ensureVaultForWorld(C);let r=e.vaultPayload()||{},c=r.facets||r.folders||[],w=[];for(let F of c)for(let U of F.documents||[])e.isMarkdownFilename(U.filename||U.github_path)&&w.push(U);let I=e.$("#vault-picker-list"),M=e.$("#vault-picker-dialog");!I||!M||(I.innerHTML=w.length?w.map(F=>`
      ...
      </div>`).join(""):""}async function W(C,{direct:r=!1,specId:c=""}={}){let w=e.chatPayload({message:C});r&&c&&(w.specialist=c);let I=await e.api("/chat/async",{method:"POST",body:JSON.stringify(w),timeoutMs:2e4});e.state._chatAttachments=[];let M=I.job;e.chatHistory.push({role:"agent",text:"",pending:!0,jobId:M.id,pendingLabel:M.phase||"Starting\u2026"}),localStorage.setItem("fos_chat",JSON.stringify(e.chatHistory)),e.state._activeJob=M,e.render(),e.startLivePoll();try{let F=await e.pollAgentJob(M.id);F?.job?.session_id&&e.setChatSessionId(F.job.session_id),F?.pending_approvals&&(e.state.approvals=F.pending_approvals,e.updateBadges()),localStorage.setItem("fos_chat",JSON.stringify(e.chatHistory)),await e.loadChatSessionsList()}finally{e.state._activeJob=null,e.pollLive(),e.currentView==="chat"&&e.render()}}async function N(C){let r=C||e.state._activeJob?.id;if(r)try{await e.api(`/chat/jobs/${encodeURIComponent(r)}/cancel`,{method:"POST",timeoutMs:1e4}),e.state._activeJob?.id===r?await e.pollAgentJob(r):e.pollLive()}catch(c){alert(c.message)}}function q(){let C=e.state._agents||{},r=e.routingMeta(C),c=e.routingLabel(C),w=e.isDirectSpecialist(),I=e.listSpecialists(C),M=e.state.ragMode||"auto",F=e.RAG_MODES.find(V=>V.id===M)||e.RAG_MODES[0],U=e.renderChatMessagesInner(),m=e.state.live||{},_=!e.chatHistory.length,P=!!e.state._activeJob?.active||e.chatHistory.some(V=>V.pending),E=e.collectAgentRuns().slice(0,4);return`<div class="chat-shell">
      <header class="chat-header driver-card">
        ...
        <div class="chat-header__meta">
          ...
          ${P?'<span class="badge-pill badge-pill--alert">Working</span>':""}
      ...
      <div class="chat-layout chat-layout--rich">
        <div class="chat-wrap">
          <div class="chat-messages${_?" is-empty":""}" id="chat-messages">
            ${_?`<div class="chat-empty">
          ...
          <div class="chat-composer driver-card">
            ...
            <div class="chat-input-row">
              <textarea class="text-input-on-dark chat-input" id="chat-input" placeholder="${w?`Task for ${e.esc(r.label)}\u2026`:"Message supervisor\u2026"}" rows="3"${P?" disabled":""}></textarea>
              <button class="button-primary" id="chat-send"${P?" disabled":""}>${w?`Run ${e.esc(r.label)}`:"Send"}</button>
            </div>
            <div class="chat-toolbar">
              ...
              ${P?'<button type="button" class="button-outline-on-dark button-sm" data-cancel-active-job>Stop</button>':""}
              <button type="button" class="button-outline-on-dark button-sm" data-goto="world">Worlds</button>
            </div>
          </div>
        ...
        <aside class="chat-rail">
          ${e.renderLivePanel(m,"chat-live-panel")}
          <section class="driver-card chat-rail-card">
            <p class="caption-uppercase">Specialists</p>
            <div class="specialist-chips" style="margin-top:var(--space-xxs)">${I.map(V=>`<span class="specialist-chip${e.currentSpecialistId()===V.id?" is-selected":""}${e.agentBusy(m,V.id)?" is-busy":""}">${e.esc(V.label)}</span>`).join("")}</div>
          </section>
          ${E.length?`<section class="driver-card chat-rail-card">
            <p class="caption-uppercase">Recent runs</p>
            <div class="activity-timeline">${E.map(V=>`<div class="activity-timeline__row"><span>${e.esc((V.agent||"").toUpperCase())}</span><span class="muted">${e.esc((V.task||"").slice(0,40))}</span></div>`).join("")}</div>
          </section>`:""}
        </aside>
      </div>
    </div>`}function z(){requestAnimationFrame(()=>{let C=e.$("#chat-messages")?.querySelectorAll(".msg:not(.system)"),r=C?.[C.length-1];FOSMotion?.animateNewMessage?.(r)})}async function Z(){try{await e.api("/auth/logout",{method:"POST",body:"{}"})}catch{}e.showPinGate()}async function ae(){let C=e.$("#chat-input"),r=(C?.value||"").trim();if(!r||e.chatHistory.some(U=>U.pending))return;let c=e.currentSpecialistId(),w=e.routingMeta(e.state._agents||{}),I=!!c;C.value="",e.chatHistory.push({role:"user",text:r}),localStorage.setItem("fos_chat",JSON.stringify(e.chatHistory)),e.render(),e.animateLatestChatMessage();let M=e.$("#chat-send"),F=I?`Run ${w.label}`:"Send";M&&(M.disabled=!0,M.textContent="\u2026");try{await e.startAgentJob(r,{direct:I,specId:c})}catch(U){e.chatHistory.push({role:"system",text:"Error: "+U.message}),localStorage.setItem("fos_chat",JSON.stringify(e.chatHistory)),e.render()}M&&(M.disabled=!1,M.textContent=F),e.animateLatestChatMessage()}async function J(){let C=!e.state.config?.agent_paused;await e.api("/agent/pause",{method:"POST",body:JSON.stringify({paused:C})}),await e.refresh(),e.render()}e.chatSessionId=S,e.setChatSessionId=k,e.applyChatSessionResponse=b,e.loadChatFromServer=a,e.chatPayload=p,e.renderMessageHtml=u,e.msgExpandKey=h,e.msgReadLineLimit=n,e.initMsgReadMore=t,e.renderArtifactLinks=s,e.loadChatSessionsList=l,e.renderChatSessionsList=i,e.openMdEditor=g,e.renderChatAttachmentChips=T,e.openVaultAttachPicker=A,e.pollAgentJob=v,e.patchChatJobBubble=y,e.renderChatMessagesInner=L,e.startAgentJob=W,e.cancelActiveJob=N,e.renderChat=q,e.animateLatestChatMessage=z,e.logoutPin=Z,e.sendChat=ae,e.togglePause=J}function _e(e){function S(t){t!=null&&(e.state._documentsSelectedId=Number(t)),e.goView("documents")}function k(){let t=e.state._artifacts||[],s=e.state._documentsSelectedId,l=t.find(v=>v.id===s),i=e.state._documentDraft??"",g=e.documentsEditMode,T=t.length?t.map(v=>`
...
`,world_id:s&&s!=="root"?s:null}),timeoutMs:15e3});e.state._documentsSelectedId=l.artifact?.id,e.documentsEditMode=!0,await e.loadViewData("documents"),e.render()}async function a(t){if(!t)return;let s=new FormData;s.append("file",t);let l=e.currentWorldId();l&&l!=="root"&&s.append("world_id",l);let i=await e.apiUpload("/artifacts",s);e.state._documentsSelectedId=i.artifact?.id,e.documentsEditMode=!1,await e.loadViewData("documents"),e.render()}async function p(){let t=e.state._documentsSelectedId;if(!t)return;let s=document.getElementById("docs-source")?.value??e.state._documentDraft??"",l=document.getElementById("docs-title-input")?.value??"Untitled",i=document.getElementById("docs-world-select")?.value??"root";await e.api(`/artifacts/${t}/content`,{method:"PUT",body:JSON.stringify({content:s}),timeoutMs:15e3}),await e.api(`/artifacts/${t}`,{method:"PATCH",body:JSON.stringify({title:l,world_id:i==="root"?null:i}),timeoutMs:15e3}),e.state._documentDraft=s,e.documentsEditMode=!1,await e.loadViewData("documents"),e.render()}async function u(){let t=e.state._documentsSelectedId;if(!t)return;e.documentsEditMode&&await e.saveCurrentDocument();let s=await e.api(`/artifacts/${t}/memory`,{method:"POST",body:"{}",timeoutMs:2e4});alert(`Saved to memory (${s.collection||"documents"}).`)}async function h(t){e.state._documentsSelectedId=Number(t),e.documentsEditMode=!1;try{let s=await e.api(`/artifacts/${t}/content`,{timeoutMs:15e3});e.state._documentDraft=s.content||""}catch(s){e.state._documentDraft="",alert(s.message||"Could not load document")}e.render()}function n(t){let s=(t||"").toLowerCase();return s.endsWith(".md")||s.endsWith(".markdown")||s.endsWith(".rst")}e.openDocumentsWorkspace=S,e.renderDocuments=k,e.createNewDocument=b,e.uploadDocumentFile=a,e.saveCurrentDocument=p,e.saveDocumentToMemory=u,e.selectDocument=h,e.isMarkdownFilename=n}function $e(e){function S(r){let c=r?.supervisor||{};return{id:"supervisor",label:"Supervisor",role:"aggregator",tool_count:r?.total_tools,brief:c.role||"Orchestrates specialists \u2014 picks who to run when routing is Auto"}}function k(r){let c=r?.specialists||[];return(c.length?c:e.DEFAULT_SPECIALISTS).map(I=>({...I,label:I.label||I.id}))}function b(){let r=e.listSpecialists(e.state._agents||{}),c=e.state.selectedSpecialist??"";c&&!r.some(U=>U.id===c)&&(c=""),e.state.selectedSpecialist=c;let I=`<option value="">Auto \u2014 supervisor decides</option>${r.map(U=>`<option value="${e.esc(U.id)}">${e.esc(U.label)}</option>`).join("")}`,M=e.$("#specialist-select-agents");M&&(M.innerHTML=I,M.value=c);let F=e.$("#chat-specialist-select");F&&(F.innerHTML=I,F.value=c)}function a(r){let c=e.currentSpecialistId();return c?`Supervisor \u2192 ${e.listSpecialists(r||e.state._agents||{}).find(I=>I.id===c)?.label||c}`:"Supervisor \xB7 auto-route"}function p(r){let c=e.state._agents||r||{},w=e.currentSpecialistId();return w?e.listSpecialists(c).find(I=>I.id===w)||{id:w,label:w,role:"specialist"}:e.supervisorMeta(c)}function u(r,c){let w=r?.jobs||[],I=String(c||"");if(w.some(F=>F.status==="running"&&(F.specialist===I||I==="supervisor"&&F.mode==="chat")))return!0;let M=r?.active?String(r.actor||""):"";return I==="supervisor"?M==="user":M===`subagent:${I}`||I&&M.includes(I)}function h(r){let c=e.AGENT_ROLES[r]||{label:r||"Specialist",cls:""};return`<span class="agent-role-badge ${c.cls}">${e.esc(c.label)}</span>`}function n(r,c){let w=e.AGENT_ROLES[c]||e.AGENT_ROLES.aggregator,I=e.AGENT_INITIALS[r]||(r||"??").slice(0,2).toUpperCase();return`<span class="agent-avatar ${w.avatar||"agent-avatar--aggregator"}" aria-hidden="true">${e.esc(I)}</span>`}function t(r,c){let w=(c||[]).find(M=>M.agent===r);return w?.ts?new Date(typeof w.ts=="number"&&w.ts<1e12?w.ts*1e3:w.ts).toLocaleString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}):""}function s(){let r=e.state._agentRunsApi||[],w=[...e.readJsonStorage("fos_agent_runs",[])];for(let I of r)w.some(M=>M.id===I.id)||w.push({...I,source:"trace"});return w.sort((I,M)=>(M.ts||0)-(I.ts||0)),w.slice(0,50)}function l(r){let c=e.readJsonStorage("fos_agent_runs",[]);c.unshift(r),localStorage.setItem("fos_agent_runs",JSON.stringify(c.slice(0,50)))}function i(r){let c=!e.currentSpecialistId();return`<button type="button" class="fleet-card fleet-card--auto${c?" is-selected":""}" data-select-specialist="" aria-pressed="${c}">
    ...
    </button>`}function g(r,c){let w=e.supervisorMeta(r),I=e.agentBusy(c,"supervisor");return`<div class="supervisor-banner driver-card">
    ...
    </div>`}function T(r,c,w,I){let M=e.agentBusy(c,r.id),F=w===r.id,U=e.lastRunForAgent(r.id,I);return`<button type="button" class="fleet-card${M?" is-busy":""}${F?" is-selected":""}" data-select-specialist="${e.esc(r.id)}" aria-pressed="${F}">
      ${F?'<span class="fleet-card__active-label">Direct</span>':""}
      <div class="fleet-card__top">
        ${e.agentAvatar(r.id,r.role)}
        <span class="fleet-card__status ${M?"is-busy":""}" title="${M?"Working":"Idle"}"></span>
      </div>
    ...
    </button>`}function A(r,c,w=!1){let I=e.listSpecialists(r),M=e.currentSpecialistId(),F=e.collectAgentRuns();return w?`<div class="fleet-rail">${e.renderFleetAutoCard(c)}${I.map(U=>e.renderFleetCard(U,c,M,F)).join("")}</div>`:`<div class="agent-grid">${I.map(U=>{let m={...U,label:U.label||U.id};return`<article class="agent-card${e.agentBusy(c,U.id)?" is-busy":""}">
          <div class="agent-card-head">${e.renderFleetCardInner(m,c,F)}</div>
        </article>`}).join("")}</div>`}function v(r,c,w){let I=e.agentBusy(c,r.id),M=e.lastRunForAgent(r.id,w);return`
      <div class="agent-card-title-row">
        ${e.agentAvatar(r.id,r.role)}
        <div><h3>${e.esc(r.label)}</h3>${r.role?e.agentRoleBadge(r.role):""}</div>
      </div>
      <span class="agent-status ${I?"busy":"ready"}">${I?"Working":"Ready"}</span>
      <p class="agent-meta">${r.tool_count??0} tools${M?` \xB7 ${e.esc(M)}`:""}</p>`}function y(r){return r.length?`<div class="table-wrap"><table class="data-table">
      <thead><tr><th>Time</th><th>Agent</th><th>Task</th><th>Duration</th><th>Tools</th><th></th></tr></thead>
      <tbody>${r.map(c=>{let w=c.ts?e.fmtTime(c.ts):"\u2014",I=(c.tools||[]).slice(0,4).join(", "),M=e.state.expandedRunId===c.id;return`<tr class="data-row${M?" is-expanded":""}" data-run-id="${e.esc(c.id)}">
          ...
          <td><button type="button" class="button-tertiary-text button-sm" data-toggle-run="${e.esc(c.id)}">${M?"Hide":"View"}</button></td>
        </tr>
        ${M?`<tr class="data-row-detail"><td colspan="6"><pre class="run-result mono">${e.esc(c.result||"No output recorded")}</pre></td></tr>`:""}`}).join("")}</tbody>
    </table></div>`:'<div class="empty-state"><p class="title-sm">No specialist runs yet</p></div>'}function L(){let r=e.state._tools||{},c=r.by_category||{};return`<div class="console-split">
      <div class="driver-card">${Object.entries(c).sort((I,M)=>M[1]-I[1]).map(([I,M])=>`<div class="kv-row"><span class="k">${e.esc(I)}</span><span class="v">${M}</span></div>`).join("")||"<p class='muted'>No tools loaded</p>"}</div>
    ...
    </div>`}function W(){let r=e.state._crm||{},c=r.pipeline||{},w=r.contacts||[],I=r.followups_due||[],M=Object.entries(c).map(([m,_])=>`<div class="kv-row"><span class="k">${e.esc(m)}</span><span class="v">${_}</span></div>`).join(""),F=I.slice(0,8).map(m=>`<li>${e.esc(m.name)} <span class="muted">${e.esc(m.company||"")}</span></li>`).join("")||"<li class='muted'>None due</li>",U=w.slice(0,10).map(m=>`<tr><td>${e.esc(m.name)}</td><td>${e.esc(m.company||"\u2014")}</td><td>${e.esc(m.status||"\u2014")}</td></tr>`).join("");return`<div class="console-split">
      <section class="driver-card"><p class="caption-uppercase">Pipeline</p>${M||"<p class='muted'>Empty</p>"}
    ...
    </div>`}function q(){let r=e.state.agentsTab||"runs",c=e.collectAgentRuns();if(r==="runs")return e.renderAgentRunsTable(c);if(r==="live"){let w=e.state.live||{};return e.renderLivePanel(w,"agents-tab-live")}return r==="tools"?e.renderAgentsToolsPanel():r==="crm"?e.renderAgentsCrmPanel():r==="vault"?e.renderAgentsVaultPanel():""}function z(){let r=e.state._agents||{},c=e.state.live||r.live||{},w=e.routingMeta(r),I=e.routingLabel(r),M=e.isDirectSpecialist(),F=e.state._delegateDraft||"",U=e.collectAgentRuns(),m=(e.state.approvals||[]).length,_=(r.specialists||[]).filter(K=>e.agentBusy(c,K.id)).length,P=r.skills||[],E=e.state.agentsTab||"runs",V=!!(e.state._delegateResult||"").trim(),Q=e.state._agentActions||[];return`<div class="agents-console">
      <header class="console-toolbar driver-card">
        <div class="console-kpis">
          ...
          <div class="console-kpi"><span class="console-kpi__val">${_||"0"}</span><span class="console-kpi__lbl">Active</span></div>
          ...
          <div class="console-kpi"><span class="console-kpi__val">${m}</span><span class="console-kpi__lbl">Approvals</span></div>
        </div>
        <div class="console-toolbar__actions">
          <span class="badge-pill" data-active-world-label>${e.esc(e.activeWorldLabel())}</span>
          ${P.map(K=>`<span class="skill-chip${K.installed?"":" is-missing"}">${e.esc(K.name)}</span>`).join("")}
          <button type="button" class="button-outline-on-dark button-sm" data-goto="chat">Chat</button>
          <button type="button" class="button-outline-on-dark button-sm" data-goto="approvals"${m?"":" disabled"}>Approvals${m?` (${m})`:""}</button>
        </div>
      </header>
      ...
      <div class="agents-workspace">
        <section class="task-composer driver-card">
          <div class="task-composer__head">
            <div class="agent-card-title-row">
              ${e.agentAvatar(M?w.id:"supervisor",M?w.role:"aggregator")}
              <div>
                <h2 class="title-md">${M?e.esc(w.label):"Supervisor"}</h2>
                <p class="world-meta">${M?e.esc((w.brief||"").slice(0,100)):"Auto-route \u2014 supervisor will delegate to the best specialist"}</p>
              </div>
            </div>
            <span class="agent-status ${e.agentBusy(c,M?w.id:"supervisor")?"busy":"ready"}">${e.esc(I)}</span>
          </div>
          <textarea class="text-input-on-dark task-composer__input" id="delegate-selected" rows="3" placeholder="${M?`Task for ${e.esc(w.label)}\u2026`:"Message supervisor\u2026"}">${e.esc(F)}</textarea>
          <div class="task-composer__foot">
            <button type="button" class="button-primary" id="delegate-selected-btn">${M?`Run ${e.esc(w.label)}`:"Send to supervisor"}</button>
            <span class="world-meta mono" data-active-world-label>${e.esc(e.activeWorldLabel())}</span>
          </div>
          ${V?`<div class="delegate-result-wrap msg-read-more-host driver-card" data-msg-scope="agents-delegate" data-msg-index="0">
        ...
        <aside class="agents-rail driver-card">
          ...
          <div class="action-feed">${Q.slice(0,8).map(K=>`<div class="action-feed__item"><span class="mono">${e.esc(K.tool_name)}</span><span class="muted">${e.esc((K.created_at||"").slice(11,16))}</span></div>`).join("")||"<p class='muted'>No actions yet</p>"}</div>
        </aside>
      </div>
  
      <section class="driver-card agents-panel">
        <div class="workspace-tabs">
          <button type="button" class="workspace-tab${E==="runs"?" is-active":""}" data-agents-tab="runs">Run history</button>
          <button type="button" class="workspace-tab${E==="live"?" is-active":""}" data-agents-tab="live">Live runtime</button>
          <button type="button" class="workspace-tab${E==="tools"?" is-active":""}" data-agents-tab="tools">Tools</button>
          <button type="button" class="workspace-tab${E==="crm"?" is-active":""}" data-agents-tab="crm">CRM</button>
          <button type="button" class="workspace-tab${E==="vault"?" is-active":""}" data-agents-tab="vault">Vault</button>
        </div>
        <div class="agents-tab-body">${e.renderAgentsTabPanel()}</div>
      </section>
    </div>`}function Z(){if(e.currentView!=="agents"||e.state.agentsTab!=="vault")return;let r=document.querySelector(".agents-console .console-split");r&&(r.outerHTML=e.renderAgentsVaultPanel())}function ae(r){let c=r||"";e.state.selectedSpecialist=c,localStorage.setItem("fos_selected_specialist",c),e.populateSpecialistSelect(),e.render()}async function J(){let r=e.$("#agents-vault-q")?.value?.trim();e.state._agentsVaultQ=r;let c=e.$("#agents-vault-results"),w=e.currentWorldId();if(!(!r||!w||w==="root"))try{let M=((await e.api(`/vault/search?${new URLSearchParams({q:r,world_id:w})}`)).hits||[]).map(F=>`[${F.metadata?.domain||"?"}] ${F.metadata?.source||""}
${(F.text||"").slice(0,240)}`).join(`

---

`)||"No hits.";c&&(c.textContent=M,c.hidden=!1)}catch(I){c&&(c.textContent=I.message,c.hidden=!1)}}async function C(){let r=e.currentSpecialistId(),c=e.$("#delegate-selected"),w=(c?.value||"").trim();if(!w)return;let I=e.$("#delegate-selected-btn"),M=e.routingMeta(e.state._agents||{}),F=!!r,U=Date.now();I&&(I.disabled=!0,I.textContent="Running\u2026"),e.startLivePoll(),e.state.agentsTab="live",localStorage.setItem("fos_agents_tab","live"),e.state._delegateResult="Agent working\u2026",e.render();try{let m=await e.api("/chat/async",{method:"POST",body:JSON.stringify(e.chatPayload({message:w,specialist:F?r:void 0})),timeoutMs:2e4}),_=await e.pollAgentJob(m.job.id),P=_?.job,E=P?.result||P?.error||"(no response)";e.state._delegateResult=E,e.state._delegateDraft="",c&&(c.value=""),P?.session_id&&e.setChatSessionId(P.session_id),e.persistAgentRun({id:P?.run_id||`local-${U}`,agent:F?r:"supervisor",task:w,result:E,duration_s:P?.elapsed_s||Math.round((Date.now()-U)/1e3),ts:Math.floor(U/1e3),tools:(P?.events||[]).filter(V=>V.name).map(V=>V.name),source:"delegate",artifacts:P?.artifacts}),e.state.agentsTab="runs",localStorage.setItem("fos_agents_tab","runs"),e.state.expandedRunId=P?.run_id||`local-${U}`,_?.pending_approvals&&(e.state.approvals=_.pending_approvals,e.updateBadges())}catch(m){e.state._delegateResult="Error: "+m.message}I&&(I.disabled=!1,I.textContent=F?`Run ${M.label}`:"Send to supervisor");try{let m=await e.api("/agents/runs");e.state._agentRunsApi=m.runs||[],e.state._agentActions=m.actions||[]}catch{}e.state._activeJob=null,e.pollLive(),e.render(),e.drawGraphs()}e.supervisorMeta=S,e.listSpecialists=k,e.populateSpecialistSelect=b,e.routingLabel=a,e.routingMeta=p,e.agentBusy=u,e.agentRoleBadge=h,e.agentAvatar=n,e.lastRunForAgent=t,e.collectAgentRuns=s,e.persistAgentRun=l,e.renderFleetAutoCard=i,e.renderSupervisorBanner=g,e.renderFleetCard=T,e.renderAgentCards=A,e.renderFleetCardInner=v,e.renderAgentRunsTable=y,e.renderAgentsToolsPanel=L,e.renderAgentsCrmPanel=W,e.renderAgentsVaultPanel=N,e.renderAgentsTabPanel=q,e.renderAgents=z,e.patchAgentsVaultPanel=Z,e.selectSpecialist=ae,e.agentsVaultSearch=J,e.delegateAgent=C}function Se(e){function S(o){let d=e.state.worlds||e.state._worldFull?.worlds||{},f=d.root,$=d.children||[],R=o||"",G=`<option value="root"${R==="root"||!R?" selected":""}>${e.esc(f?.name||"Main world")}</option>`;return G+=$.map(O=>`<option value="${e.esc(O.id)}"${R===O.id?" selected":""}>${e.esc(O.name)} \xB7 ${e.esc(O.kind||"project")}</option>`).join(""),G}function k(o,d){let f=o?.facets||o?.folders||[],$=[];for(let R of f)for(let G of R.documents||[])G.github_repo===d&&$.push(G);return $.sort((R,G)=>(R.github_path||R.filename||"").localeCompare(G.github_path||G.filename||""))}function b(o){let d=o.filter(f=>{let $=f.github_path||f.filename||"";return/^readme\.md$/i.test($.split("/").pop()||"")});return d.length?d.sort((f,$)=>(f.github_path||f.filename||"").length-($.github_path||$.filename||"").length)[0]:null}function a(o){let d=(o.files||[]).length;for(let f of Object.keys(o.dirs||{}))d+=e.countGithubTreeFiles(o.dirs[f]);return d}function p(o,d,f=0){let $=Object.keys(o.dirs||{}).sort(),R=(o.files||[]).sort((O,j)=>O._fileName.localeCompare(j._fileName)),G="";for(let O of $){let j=o.dirs[O],Y=e.countGithubTreeFiles(j);G+=`<details class="github-tree-dir"${f<2?" open":""}>
        <summary><span class="mono">${e.esc(O)}</span> <span class="muted">${Y} file${Y!==1?"s":""}</span></summary>
        <div class="github-tree">${e.renderGithubTreeNode(j,d,f+1)}</div>
      </details>`}for(let O of R){let j=O.github_path||O.filename||O.title,Y=/^readme\.md$/i.test((j||"").split("/").pop()||"");G+=`<div class="github-tree-file">
        <span class="github-tree-file__path mono${Y?" is-readme":""}">${e.esc(j)}</span>
        <span class="github-tree-file__actions">
          <button type="button" class="button-outline-on-dark button-sm" data-vault-view-doc="${O.id}" data-world-id="${e.esc(d)}" data-doc-title="${e.esc(O.title||j)}">View</button>
          <button type="button" class="button-primary button-sm" data-tag-vault-doc="${O.id}" data-world-id="${e.esc(d)}" data-doc-title="${e.esc(O.title||j)}" data-doc-path="${e.esc(j)}">Tag in agent</button>
        </span>
      </div>`}return G}function u(o,d,f,$){e.state._chatAttachments||(e.state._chatAttachments=[]);let R=Number(o);e.state._chatAttachments.some(G=>G.doc_id===R)||e.state._chatAttachments.push({type:"vault",doc_id:R,title:f||$||"Document",path:$||"",world_id:d}),e.goView("chat")}function h(o,d){if(o?.nodes&&o?.edges)return o;let f=o?.vault||o||{},$=d||{},R=[],G=[],O=$.id||f.world_id||"world",j=`vault-world:${O}`;return R.push({data:{id:j,label:($.name||"World").slice(0,36),type:"world_root",world_id:O}}),(f.facets||f.folders||[]).forEach(D=>{let X=D.id||D.folder||"slot",B=`vault-facet:${O}:${X}`,te=`${D.label||D.folder||"Folder"} (${D.file_count||0})`;R.push({data:{id:B,label:te.slice(0,40),type:"vault_facet",facet_id:X,folder:D.folder}}),G.push({data:{source:j,target:B,label:"folder"}}),(D.documents||[]).slice(0,14).forEach((x,ee)=>{let se=`vault-doc:${x.id||ee}`;R.push({data:{id:se,label:(x.title||x.filename||"Document").slice(0,36),type:"vault_file",doc_id:x.id,facet_id:X,source:x.source_type||"upload"}}),G.push({data:{source:B,target:se,label:"doc"}})}),(D.files||[]).slice(0,8).forEach((x,ee)=>{let se=`vault-disk:${O}:${X}:${ee}`;R.push({data:{id:se,label:(x.name||x.relative||"file").slice(0,32),type:"vault_file",path:x.relative,facet_id:X,source:"disk"}}),G.push({data:{source:B,target:se,label:"disk"}})})}),(f.github_repos||[]).slice(0,10).forEach(D=>{let X=`gh-repo:${D.id}`;R.push({data:{id:X,label:(D.full_name||"repo").split("/").pop().slice(0,28),type:"vault_repo",link_id:D.id,repo:D.full_name}}),G.push({data:{source:j,target:X,label:"github"}})}),R.length<=1&&(R.push({data:{id:"vault-empty",label:"Add docs or link GitHub",type:"empty"}}),G.push({data:{source:j,target:"vault-empty",label:"start"}})),{nodes:R,edges:G}}function n(o){let d=o?.id;if(!d||d==="root")return{nodes:[],edges:[]};if(e.state._vaultLoading&&e.state._vaultWorldId!==d)return{nodes:[{data:{id:"vault-loading",label:"Loading vault\u2026",type:"loading"}}],edges:[]};if(e.state._vaultWorldId===d&&e.state._vaultGraph?.nodes?.length)return e.state._vaultGraph;let f=e.vaultReadyFor(d)?e.vaultPayload():null;return f?e.buildVaultGraph(f,o):e.state._vaultLoading?{nodes:[{data:{id:"vault-loading",label:"Loading vault\u2026",type:"loading"}}],edges:[]}:{nodes:[{data:{id:"vault-empty",label:"Vault not loaded",type:"empty"}}],edges:[]}}function t(o){return o==="vault"?`
      ...
      </form>`}function l(o){let d=e.worldTreeData(),f=o||"root";return f==="root"||f===d.root?.id?d.root||null:(d.children||[]).find($=>$.id===f)||null}function i(){return e.state.inspectorWorldId||e.currentWorldId()||"root"}async function g(o,{force:d=!1}={}){if(!o||o==="root"){e.clearVaultScopedState(),e.invalidateGraphCache("graph-world");return}if(!d&&e.vaultReadyFor(o))return;let f=++e.vaultLoadGen;e.state._vaultLoading=!0,e.state._vaultWorldId=o,e.currentView==="world"&&e.patchWorldPanels();try{let $=await e.api(`/worlds/${encodeURIComponent(o)}/vault`);if(f!==e.vaultLoadGen)return;e.state._worldVault=$.vault||null,e.state._vaultGraph=$.vault_graph||null,e.state._vaultWorldId=o,e.invalidateGraphCache("graph-world")}catch{if(f!==e.vaultLoadGen)return;e.clearVaultScopedState()}finally{f===e.vaultLoadGen&&(e.state._vaultLoading=!1)}}async function T(o,d={}){if(!o||o==="root"){e.clearVaultScopedState();return}d.force&&(e.state._vaultWorldId=null),await e.loadWorldVault(o,{force:!0})}async function A(){try{let o=await e.api("/graph/world");e.state._worldFull=o,e.state._worldGraph=o?.graph??null,e.state._worldHierarchyGraph=o?.hierarchy_graph??null,e.state._worldPreviews=o?.world_previews??{},o?.worlds&&(e.state.worlds=o.worlds),e.populateWorldSelect(),e.invalidateGraphCache("graph-world")}catch(o){console.warn("world tree reload failed:",o)}}async function v(o,d={}){if(!o||o==="root"){e.clearVaultScopedState();return}!d.force&&e.vaultReadyFor(o)||await e.loadWorldVault(o,{force:!!d.force})}function y(){let o=e.inspectorWorldId(),d=e.state.activeWorldId||"root";e.$$("[data-inspect-world]").forEach($=>{let R=$.dataset.inspectWorld;$.classList.toggle("is-inspect",R===o),$.classList.toggle("is-active",R===d)});let f=document.querySelector(".worlds-stat [data-active-world-label]");f&&(f.textContent=e.activeWorldLabel())}function L(){if(e.currentView!=="world")return;let o=e.inspectorWorldId(),d=e.worldById(o),f=e.state._worldFull?.snapshot||e.state.snapshot||{},$=document.getElementById("world-inspector");$&&($.innerHTML=e.renderWorldInspector(d,f));let R=document.getElementById("world-vault-mount");if(e.isRootWorld(d))R&&(R.innerHTML="");else{let G=e.renderWorldVaultPanel(d);R&&(R.innerHTML=G)}e.patchWorldTreeNav(),e.drawGraphs()}async function W(o={}){let d=e.currentWorldId(),f=e.inspectorWorldId(),$=o.vaultWorldId||(e.currentView==="world"?f:d);!$||$==="root"?e.clearVaultScopedState():await e.ensureVaultForWorld($,{force:!!o.forceVault}),e.currentView==="world"&&o.reloadTree?await e.reloadWorldTree():(e.currentView==="world"||e.currentView==="dashboard")&&await e.loadGraphData(),e.drawGraphs()}function N(o){let d=o||"root";e.inspectorWorldId()===d&&e.vaultReadyFor(d)&&!e.state._vaultLoading||(e.state.inspectorWorldId=d,e.currentView==="world"&&(e.state._motionSkipOnce=!0,e.state.ui||(e.state.ui={}),e.state.ui.vaultFacet=null,e.clearVaultScopedState(),e.invalidateGraphCache("graph-world"),e.patchWorldPanels(),e.reloadVault(d,{force:!0}).then(()=>{e.patchWorldPanels(),FOSMotion?.flashElement?.(e.$("#world-inspector")),window.FOSGraph?.highlightWorld("graph-world",e.inspectorWorldId(),e.currentWorldId())}).catch(console.error)))}function q(o,d,f,$){let R=o?.id||"root",G=`
      <button type="button" class="world-tree-item is-root${f===R?" is-inspect":""}${$===R?" is-active":""}"
      ...
      <button type="button" class="world-tree-item kind-${e.esc(j.kind||"project")}${f===j.id?" is-inspect":""}${$===j.id?" is-active":""}"
        data-inspect-world="${e.esc(j.id)}">
        <span class="dot" aria-hidden="true"></span>
        <span class="meta">
          <span class="name">${e.esc(j.name)}</span>
          <span class="sub">${e.esc(j.kind||"project")} \xB7 ${e.esc((j.description||"No description").slice(0,42))}</span>
        </span>
      </button>`).join("");return`
      <nav class="world-tree-nav" aria-label="World hierarchy">
        ${G}
        ${d.length?`<div class="world-tree-children">${O}</div>`:""}
      </nav>`}function z(o,d){if(!o)return'<p class="body-md muted">Select a world to inspect its context.</p>';let f=o.id||"root",$=f==="root",R=$?"root":o.kind||"project",G=e.currentWorldId(),j=(e.state._worldPreviews||e.state._worldFull?.world_previews||{})[f]||"",Y=d?.crm||{},D=d?.finance||{};if(e.state.worldEditing===f)return`
        ...
        </form>`;let B=$?[["Contacts",Y.total_contacts||0],["Follow-ups",Y.followups_due||0],["Open tasks",d?.tasks_open||0],["Approvals",d?.approvals_pending||0]]:[];$&&D?.set&&B.push(["Runway",D.runway_months!=null?`${D.runway_months} mo`:"\u2014"]);let te=$?e.worldTreeData().children||[]:[],x=(d?.goals_active||[]).slice(0,5);return`
      <div class="world-inspector-title">
        <div>
          <h2>${e.esc(o.name)}</h2>
      ...
      ${B.length?`
        <div class="world-inspector-section">
          ...
          <div class="world-inspector-facts">${B.map(([ee,se])=>`<div class="world-inspector-fact"><span class="k">${e.esc(ee)}</span><span class="v">${e.esc(String(se))}</span></div>`).join("")}</div>
        </div>`:""}
      ${$&&te.length?`
        <div class="world-inspector-section">
      ...
      <div class="world-inspector-section">
        ...
        <pre class="world-context-preview">${e.esc(j||"Preview loads when graph data is fetched\u2026")}</pre>
      </div>
      <div class="world-inspector-actions">
        ...
        ${$?"":`<button type="button" class="button-tertiary-text button-sm" data-delete-world="${e.esc(f)}">Delete</button>`}
      </div>`}function Z(o,d,f){let $=e.state.ui?.vaultDocEdit,R=f||d[0]?.id||d[0]?.folder||"docs",G=d.find(D=>(D.id||D.folder)===R)||d[0]||{label:R,id:R},O=$&&$.title||"",j=$&&$.description||"",Y=$?.id||"";return`
      <form class="human-form vault-doc-form" id="vault-doc-form" data-world-id="${e.esc(o.id)}" data-facet-id="${e.esc(R)}">
        ...
        <div class="human-form__row">
          <label class="human-field"><span class="caption-uppercase">Category slot</span>
            <select class="text-input-on-dark" name="facet_id" id="vault-doc-facet">
              ${d.map(D=>{let X=D.id||D.folder;return`<option value="${e.esc(X)}"${X===R?" selected":""}>${e.esc(D.label)}</option>`}).join("")}
            </select></label>
        ...
        <label class="human-field"><span class="caption-uppercase">Description (indexed for search)</span>
          <textarea class="text-input-on-dark" name="description" rows="3" placeholder="Short summary agents use to find this doc. Full content goes to ${e.esc(e.vaultStorageLabel())}.">${e.esc(j)}</textarea></label>
        ${Y?`
...
Target: \u2026"></textarea></label>`}
      ...
      </form>`}function ae(o,d){let f=e.state._githubStatus||{},$=!!f.connected,R=!!f.oauth_configured,G=d.github_repos||[],j=(e.state._githubRepos||[]).map(D=>`<option value="${e.esc(D.full_name)}">${e.esc(D.full_name)}${D.private?" (private)":""}</option>`).join(""),Y=G.map(D=>{let X=e.isLinkSyncing(D.id),B=e.githubRepoDocuments(d,D.full_name),te=e.findReadmeDoc(B),x=B.filter(se=>e.isMarkdownFilename(se.github_path||se.filename)),ee=x.length?`<div class="github-tree github-tree--repo">${e.renderGithubTreeNode(e.buildGithubPathTree(x),o.id)}</div>`:"";return`
      <div class="github-repo-row">
        <div>
          <strong class="mono">${e.esc(D.full_name)}</strong>
          ${X?'<span class="sync-badge">Syncing</span>':""}
          <span class="world-meta">${D.file_count||B.length||0} files synced${D.synced_at?` \xB7 ${e.esc(D.synced_at)}`:""}</span>
          ${D.last_error?`<span class="world-meta" style="color:var(--color-warn)">${e.esc(D.last_error)}</span>`:""}
        </div>
        <div class="github-repo-row__actions">
          <button type="button" class="button-primary button-sm" data-vault-view-doc="${te?.id||""}" data-world-id="${e.esc(o.id)}" data-doc-title="${e.esc(te?.title||`${D.full_name} README`)}"${!te||X?" disabled":""}>Open README</button>
          <button type="button" class="button-outline-on-dark button-sm${X?" is-busy":""}" data-github-sync="${D.id}" data-world-id="${e.esc(o.id)}"${X?" disabled":""}>${X?"Syncing\u2026":`Sync to ${e.esc(e.vaultStorageLabel())}`}</button>
          <button type="button" class="button-tertiary-text button-sm" data-github-unlink="${D.id}" data-world-id="${e.esc(o.id)}"${X?" disabled":""}>Unlink</button>
        </div>
        ${B.length?`<details class="github-repo-files" open>
      ...
      </div>`}).join("");return R?$?`<section class="github-repos-panel">
      ...
      </section>`}function J(o,d){let f=o.facets||o.folders||[],$=o.storage_backend||(e.vaultStorageLabel()==="S3"?"s3":"local");return`
      <div class="vault-registry-bar" role="status" aria-live="polite">
        <span class="vault-registry-chip"><span class="k">Template</span> ${e.esc(o.template_id||d.template||"startup")}</span>
        ...
        <span class="vault-registry-chip"><span class="k">Storage</span> ${e.esc($)}</span>
      ...
      </section>`;let d=e.vaultPayload()||{},f=d.facets||d.folders||[],$=d.domain_counts||{},R=e.state.ui?.vaultFacet||f[0]?.id||f[0]?.folder||null,G=e.state.ui?.vaultDocForm||e.state.ui?.vaultDocEdit,O=(f.find(B=>(B.id||B.folder)===R)||{}).documents||[],j=f.map(B=>{let te=B.id||B.folder,x=(B.documents||[]).length+(B.files||[]).length;return`<button type="button" class="vault-facet-tab${te===R?" is-active":""}" data-vault-facet="${e.esc(te)}">${e.esc(B.label)} <span class="badge-pill">${x}</span></button>`}).join(""),Y=O.map(B=>{let te=B.github_path?` \xB7 ${B.github_path}`:"",x=e.isMarkdownFilename(B.filename||B.github_path);return`
      <article class="vault-doc-card" data-doc-id="${B.id}">
        <div class="vault-doc-card__head">
          <h4>${e.esc(B.title)}</h4>
          <span class="world-meta">${e.esc(B.filename||"")}${e.esc(te)} \xB7 ${e.formatBytes(B.size_bytes)}${B.source_type==="github"?" \xB7 GitHub":""}</span>
        </div>
        <p class="body-md">${e.esc(B.description||"No description")}</p>
        <div class="vault-doc-card__actions">
          ${x?`<button type="button" class="button-primary button-sm" data-vault-view-doc="${B.id}" data-world-id="${e.esc(o.id)}" data-doc-title="${e.esc(B.title)}">View</button>`:""}
          <button type="button" class="button-outline-on-dark button-sm" data-tag-vault-doc="${B.id}" data-world-id="${e.esc(o.id)}" data-doc-title="${e.esc(B.title)}" data-doc-path="${e.esc(B.github_path||B.filename||"")}">Tag in agent</button>
          <button type="button" class="button-outline-on-dark button-sm" data-vault-edit-doc="${B.id}">Edit</button>
          <button type="button" class="button-tertiary-text button-sm" data-vault-delete-doc="${B.id}">Remove</button>
        </div>
      </article>`}).join(""),D=(f.find(B=>(B.id||B.folder)===R)||{}).files||[],X=D.length?`<ul class="vault-file-list">${D.map(B=>`<li class="mono">${e.esc(B.relative||B.name)} <span class="muted">on disk</span></li>`).join("")}</ul>`:"";return`
      <section class="driver-card vault-panel knowledge-panel" style="margin-top:var(--space-md)">
        <div class="vault-panel-head">
          <div>
            ...
            <h3 class="title-sm">${e.esc(o.name)} \u2014 ${e.esc(d.template_id||o.template||"startup")} template</h3>
        ...
        ${e.renderGithubReposPanel(o,d)}
        ${e.renderVaultRegistryBar(d,o)}
        <div class="vault-facet-tabs" role="tablist">${j||"<span class='muted'>No categories</span>"}</div>
      ...
      </section>`}function r(){let o=e.state._worldFull||{},d=o.worlds||e.state.worlds||{},f=d.root||{},$=d.children||[],R=e.inspectorWorldId(),G=e.currentWorldId(),O=e.worldById(R)||f,j=o.snapshot||e.state.snapshot||{},Y=e.state.config?.my_name||"You";e.isRootWorld(O)&&e.worldGraphTab==="vault"&&(e.worldGraphTab="hierarchy");let D=!e.isRootWorld(O);return`
          ...
          <section class="worlds-panel">
            <div class="worlds-panel-head">
              ...
              <div class="world-graph-tabs" role="tablist">
                ...
                ${D?`<button type="button" class="world-graph-tab${e.worldGraphTab==="vault"?" is-active":""}" data-world-graph-tab="vault">Files</button>`:""}
              </div>
            </div>
      ...
      </div>`}function c(o){return!o||o.id==="root"}async function w(o){let d=new FormData(o),f=(d.get("name")||"").toString().trim();if(f)try{let $=await e.api("/worlds",{method:"POST",body:JSON.stringify({name:f,kind:(d.get("kind")||"project").toString(),template:(d.get("template")||"").toString().trim()||void 0,description:(d.get("description")||"").toString().trim(),context:(d.get("context")||"").toString().trim(),repo_path:(d.get("repo_path")||"").toString().trim(),github_repo:(d.get("github_repo")||"").toString().trim()})});e.state.worlds=$.tree,e.setActiveWorld($.world?.id),await e.refresh(),e.currentView==="world"&&(await e.reloadWorldTree(),e.selectInspectorWorld($.world?.id)),o.reset(),e.state.ui&&(e.state.ui.worldCreateOpen=!1)}catch($){alert($.message)}}async function I(o){let d=o.dataset.worldId;if(!d)return;let f=new FormData(o),$={name:(f.get("name")||"").toString().trim(),description:(f.get("description")||"").toString(),context:(f.get("context")||"").toString()};if(d!=="root"){$.kind=(f.get("kind")||"project").toString();let R=(f.get("template")||"").toString().trim();R&&($.template=R)}try{let R=await e.api(`/worlds/${encodeURIComponent(d)}`,{method:"PATCH",body:JSON.stringify($)});e.state.worlds=R.tree,e.state.worldEditing=null,e.currentView==="world"?(await e.reloadWorldTree(),await e.reloadVault(d,{force:!0}),e.patchWorldPanels()):await e.refresh()}catch(R){alert(R.message)}}async function M(o){let d=o.dataset.worldId,f=(o.querySelector("[name=doc_id]")?.value||"").trim(),$=new FormData(o),R=($.get("title")||"").toString().trim(),G=($.get("facet_id")||o.dataset.facetId||"docs").toString(),O=($.get("description")||"").toString().trim(),j=($.get("content")||"").toString(),Y=o.querySelector('input[type="file"]')?.files?.[0];try{if(f)await e.api(`/worlds/${encodeURIComponent(d)}/vault/documents/${encodeURIComponent(f)}`,{method:"PATCH",body:JSON.stringify({title:R,description:O,facet_id:G,content:j||void 0})});else if(Y){let D=new FormData;D.append("file",Y),D.append("title",R),D.append("description",O),D.append("facet_id",G),await e.apiUpload(`/worlds/${encodeURIComponent(d)}/vault/documents`,D)}else if(j.trim())await e.api(`/worlds/${encodeURIComponent(d)}/vault/documents`,{method:"POST",body:JSON.stringify({title:R,description:O,facet_id:G,content:j})});else return alert("Upload a file or paste markdown content.");e.state.ui&&(e.state.ui.vaultDocForm=!1,e.state.ui.vaultDocEdit=null),await e.reloadVault(d,{force:!0}),e.afterVaultMutation(d)}catch(D){alert(D.message)}}async function F(o,d){e.state.ui||(e.state.ui={});try{let f=await e.api(`/worlds/${encodeURIComponent(o)}/vault/documents/${encodeURIComponent(d)}/content`);e.state.ui.vaultDocEdit=f.document,e.state.ui.vaultDocForm=!0,e.state.ui.vaultFacet=f.document?.facet_id||e.state.ui.vaultFacet,e.currentView==="world"?e.patchWorldPanels():e.render();let $=e.$("#vault-doc-content");$&&($.value=f.content||"")}catch(f){alert(f.message)}}async function U(o){let d=e.$("#github-repo-pick")?.value?.trim();if(!d)return alert("Select a repository");let f=document.querySelector(`[data-github-add="${o}"]`);f&&(f.disabled=!0);try{let $=await e.api(`/worlds/${encodeURIComponent(o)}/repos`,{method:"POST",body:JSON.stringify({full_name:d}),timeoutMs:12e4});if($.job?.status==="failed")throw new Error($.job.message||"Could not start sync");$.job?.id?await e.runGithubSyncJob($.job.id,`Syncing ${d}`,{worldId:o,linkId:$.repo?.id}):(await e.reloadVault(o,{force:!0}),e.afterVaultMutation(o))}catch($){alert($.message)}finally{f&&(f.disabled=e.state._syncingLinkIds.size>0)}}async function m(o,d){if(!e.isLinkSyncing(d))try{let f=await e.api(`/worlds/${encodeURIComponent(o)}/repos/${encodeURIComponent(d)}/sync`,{method:"POST",body:"{}",timeoutMs:12e4});if(f.job?.status==="failed")throw new Error(f.job.message||"Could not start sync");if(f.job?.id){let $=(e.state._worldVault?.github_repos||[]).find(R=>String(R.id)===String(d))?.full_name||"repository";await e.runGithubSyncJob(f.job.id,`Re-syncing ${$}`,{worldId:o,linkId:d})}}catch(f){alert(f.message)}}async function _(o,d){if(confirm("Unlink this repo and remove its synced documents from this world?"))try{await e.api(`/worlds/${encodeURIComponent(o)}/repos/${encodeURIComponent(d)}`,{method:"DELETE"}),await e.reloadVault(o,{force:!0}),e.afterVaultMutation(o)}catch(f){alert(f.message)}}async function P(o,d){if(confirm("Remove this document from the knowledge graph?"))try{await e.api(`/worlds/${encodeURIComponent(o)}/vault/documents/${encodeURIComponent(d)}`,{method:"DELETE"}),await e.reloadVault(o,{force:!0}),e.afterVaultMutation(o)}catch(f){alert(f.message)}}async function E(o){try{let d=await e.api(`/worlds/${encodeURIComponent(o)}/vault/ingest`,{method:"POST",body:"{}"});alert(`Ingested ${d.files||0} files (${d.total_chunks||0} chunks)`),await e.reloadVault(o,{force:!0}),e.afterVaultMutation(o)}catch(d){alert(d.message)}}async function V(o){let d=e.$("#vault-repo-path")?.value?.trim();if(!d)return alert("Enter a local repo path");try{let f=await e.api(`/worlds/${encodeURIComponent(o)}/vault/link-repo`,{method:"POST",body:JSON.stringify({repo_path:d})});if(f.error)return alert(f.error);alert(`Linked and ingested ${f.files||0} files`),await e.reloadVault(o,{force:!0}),await e.refresh(),e.afterVaultMutation(o)}catch(f){alert(f.message)}}async function Q(o){let d=e.$("#vault-search-q")?.value?.trim();if(!d)return;let f=e.$("#vault-search-results");try{let R=((await e.api(`/vault/search?${new URLSearchParams({q:d,world_id:o})}`)).hits||[]).map(G=>`[${G.metadata?.domain||"?"}] ${G.metadata?.source||""}
${(G.text||"").slice(0,200)}`).join(`

---

`)||"No hits.";f&&(f.textContent=R,f.hidden=!1)}catch($){f&&(f.textContent=$.message,f.hidden=!1)}}async function K(o){if(confirm("Delete this sub-world?"))try{let d=await e.api(`/worlds/${encodeURIComponent(o)}`,{method:"DELETE"});e.state.worlds=d.tree,e.currentWorldId()===o&&e.setActiveWorld("root"),e.inspectorWorldId()===o&&e.selectInspectorWorld("root"),await e.refresh(),e.currentView==="world"&&(e.state._worldFull=await e.api("/graph/world"),e.state._worldPreviews=e.state._worldFull?.world_previews||{},e.render())}catch(d){alert(d.message)}}e.renderWorldOptionsForDocs=S,e.githubRepoDocuments=k,e.findReadmeDoc=b,e.countGithubTreeFiles=a,e.renderGithubTreeNode=p,e.tagVaultDocInChat=u,e.buildVaultGraph=h,e.vaultGraphForWorld=n,e.worldGraphLegendHtml=t,e.renderWorldCreateForm=s,e.worldById=l,e.inspectorWorldId=i,e.loadWorldVault=g,e.reloadVault=T,e.reloadWorldTree=A,e.ensureVaultForWorld=v,e.patchWorldTreeNav=y,e.patchWorldPanels=L,e.onWorldContextChanged=W,e.selectInspectorWorld=N,e.renderWorldTreeNav=q,e.renderWorldInspector=z,e.renderVaultDocForm=Z,e.renderGithubReposPanel=ae,e.renderVaultRegistryBar=J,e.renderWorldVaultPanel=C,e.renderWorld=r,e.isRootWorld=c,e.createWorldFromForm=w,e.saveWorldEdit=I,e.submitVaultDoc=M,e.startVaultDocEdit=F,e.connectGithubRepo=U,e.syncGithubRepo=m,e.unlinkGithubRepo=_,e.deleteVaultDoc=P,e.vaultIngest=E,e.vaultLinkRepo=V,e.vaultSearch=Q,e.deleteWorld=K}function ke(e){function S(){let y=e.state.ui?.crmTab||localStorage.getItem("fos_crm_tab")||"contacts";return y==="outreach"?"contacts":y}function k(y){let L=e.state.worlds||e.state._worldFull?.worlds||{},W=L.root,N=L.children||[],q=[];return W&&q.push(`<option value="${e.esc(W.id||"root")}"${(y||"root")===(W.id||"root")?" selected":""}>${e.esc(W.name||"Main world")}</option>`),N.forEach(z=>{q.push(`<option value="${e.esc(z.id)}"${y===z.id?" selected":""}>${e.esc(z.name||z.id)}</option>`)}),q.join("")}function b(y={}){let L=e.crmTab();return`<nav class="crm-tabs" role="tablist" aria-label="CRM sections">${[["contacts","Contacts",y.contacts],["companies","Companies",y.companies],["pipeline","Pipeline",null]].map(([N,q,z])=>`<button type="button" role="tab" aria-selected="${L===N}" class="crm-tab${L===N?" crm-tab--active":""}" data-crm-tab="${N}">${e.esc(q)}${z!=null?`<span class="crm-tab__count">${z}</span>`:""}</button>`).join("")}</nav>`}function a(){let y=e.state._crm?.contacts||[],L=e.state._crm?.followups_due||[],W=!!e.state.ui?.crmFormOpen,N=e.state._crmCompanies?.companies||[],q=J=>e.CRM_STATUSES.map(C=>`<option value="${C}"${C===J?" selected":""}>${e.esc(C)}</option>`).join(""),z='<option value="">\u2014 None \u2014</option>'+N.map(J=>`<option value="${J.id}">${e.esc(J.name)}</option>`).join(""),Z=y.slice(0,50).map(J=>`<tr>
      ...
      </section>`;let y=e.state._crmCompanies?.companies||[],L=e.state._crmCompanies?.meta?.unlinked_contact_companies||0,W=!!e.state.ui?.crmCompanyFormOpen,N=e.state.ui?.crmCompanyDetail,q=e.currentWorldId(),z=r=>e.COMPANY_STATUSES.map(c=>`<option value="${c}"${c===r?" selected":""}>${e.esc(c)}</option>`).join(""),Z=y.map(r=>`<tr>
      ...
      <td class="muted">${e.esc((r.last_contacted_at||"").slice(0,10))}</td>
    </tr>`).join(""),ae="";if(N){let r=y.find(w=>String(w.id)===String(N))||e.state._crmCompanyDetail?.company,c=e.state._crmCompanyDetail?.contacts||[];r&&(ae=`<aside class="crm-company-drawer driver-card">
          <div class="human-panel__head">
          ...
          <ul class="list-plain">${c.map(w=>`<li>${e.esc(w.name)} \u2014 ${e.esc(w.role||"")} ${w.email?`<span class="muted">${e.esc(w.email)}</span>`:""}</li>`).join("")||"<li class='muted'>None</li>"}</ul>
        </aside>`)}let J=L>0?`
      ...
      <section class="driver-card span-6"><p class="caption-uppercase">Company pipeline</p><div style="margin-top:var(--space-sm)">${q}</div></section>`}function h(){let y=e.crmTab(),L={contacts:e.state._crm?.contacts?.length||0,companies:e.state._crmCompanies?.companies?.length||0},W="";return y==="contacts"?W=e.renderCrmContactsPanel():y==="companies"?W=e.renderCrmCompaniesPanel():W=e.renderCrmPipelinePanel(),`<div class="dashboard-grid">
      ...
      ${W}
    </div>`}async function n(){let y=e.crmTab(),L=e.currentWorldId(),W=y==="companies"?"?include_unassigned=1":L&&L!=="root"?`?world_id=${encodeURIComponent(L)}&include_unassigned=1`:"?include_unassigned=1";e.state._crmCompaniesLoading=!0,e.state._crmCompaniesError=null;try{let[N,q]=await Promise.all([e.api("/crm/contacts"),e.api(`/crm/companies${W}`)]);e.state._crm=N,e.state._crmCompanies=q}catch(N){e.state._crmCompaniesError=N.message||"Could not load CRM data"}finally{e.state._crmCompaniesLoading=!1}}async function t(y){let L=new FormData(y),W=(L.get("name")||"").toString().trim();if(!W)return;let N=(L.get("company_id")||"").toString().trim();try{await e.api("/crm/contacts",{method:"POST",body:JSON.stringify({name:W,company_id:N?parseInt(N,10):null,role:(L.get("role")||"").toString().trim(),email:(L.get("email")||"").toString().trim(),status:(L.get("status")||"prospect").toString(),linkedin_url:(L.get("linkedin_url")||"").toString().trim(),phone:(L.get("phone")||"").toString().trim(),whatsapp_enabled:L.get("whatsapp_enabled")==="1",notes:(L.get("notes")||"").toString().trim()})}),await e.loadCrmData(),e.state.ui&&(e.state.ui.crmFormOpen=!1),await e.refresh(),e.render(),y.reset()}catch(q){alert(q.message)}}async function s(){let y=e.currentWorldId(),L=y&&y!=="root"?y:null;try{let W=await e.api("/crm/companies/import-from-contacts",{method:"POST",body:JSON.stringify({world_id:L})});await e.loadCrmData(),e.render();let N=`Imported ${W.created||0} companies and linked ${W.linked_contacts||0} contacts.`;e.state._toast?e.state._toast(N):alert(N)}catch(W){alert(W.message)}}async function l(y){let L=new FormData(y),W=(L.get("name")||"").toString().trim(),N=(L.get("world_id")||"").toString().trim();if(!(!W||!N))try{await e.api("/crm/companies",{method:"POST",body:JSON.stringify({name:W,world_id:N,sector:(L.get("sector")||"").toString().trim(),status:(L.get("status")||"prospect").toString(),website:(L.get("website")||"").toString().trim(),linkedin_url:(L.get("linkedin_url")||"").toString().trim(),notes:(L.get("notes")||"").toString().trim()})}),await e.loadCrmData(),e.state.ui&&(e.state.ui.crmCompanyFormOpen=!1),e.render(),y.reset()}catch(q){alert(q.message)}}async function i(y){if(y)try{let L=await e.api(`/crm/companies/${encodeURIComponent(y)}`);e.state._crmCompanyDetail=L,e.state.ui||(e.state.ui={}),e.state.ui.crmCompanyDetail=y,e.render()}catch(L){alert(L.message)}}async function g(y,L){if(!(!y||!L))try{await e.api(`/crm/contacts/${encodeURIComponent(y)}`,{method:"PATCH",body:JSON.stringify({status:L})}),e.state._crm=await e.api("/crm/contacts"),await e.refresh(),e.render()}catch(W){alert(W.message)}}async function T(y,L){if(y)try{await e.api(`/crm/contacts/${encodeURIComponent(y)}`,{method:"PATCH",body:JSON.stringify({whatsapp_enabled:!!L})}),e.state._crm=await e.api("/crm/contacts"),await e.refresh(),e.render()}catch(W){alert(W.message)}}async function A(y){if(y)try{let L=await e.api(`/whatsapp/messages?contact_id=${encodeURIComponent(y)}`);e.state._crmWaThread=L.messages||[],e.render()}catch(L){alert(L.message)}}async function v(y,L){let W=parseInt(L,10)||7;await e.api(`/crm/contacts/${y}/followup`,{method:"POST",body:JSON.stringify({days:W}),timeoutMs:15e3}),e.state._crm=await e.api("/crm/contacts"),e.currentView==="crm"&&e.render()}e.crmTab=S,e.renderWorldOptionsForCrm=k,e.renderCrmTabs=b,e.renderCrmContactsPanel=a,e.renderCrmCompaniesPanel=p,e.renderCrmPipelinePanel=u,e.renderCrm=h,e.loadCrmData=n,e.submitCrmContact=t,e.importCrmCompaniesFromContacts=s,e.submitCrmCompany=l,e.openCrmCompanyDetail=i,e.updateCrmStatus=g,e.updateCrmWhatsapp=T,e.loadCrmWaThread=A,e.scheduleCrmFollowup=v}function Ce(e){function S(){return e.state.ui?.crmOutreachWorld||e.currentWorldId()}function k(){let m=e.state._crmCampaignReview,_=m?.campaign;return _?.status==="done"||m?.done&&!m?.pending_count?"complete":m?.campaign&&["review"].includes(_.status)&&m.pending_count>0?"review":m?.campaign&&["review"].includes(_.status)&&!m.pending_count?"complete":e.state._crmOutreachJob?.active||["researching","drafting","created"].includes(_?.status||e.state._crmOutreachJob?.status)||e.state.ui?.crmCampaignId&&_&&!["review","done","failed"].includes(_.status)?"running":"setup"}function b(){return e.state.ui?.crmOutreachBatch||5}function a(){return e.state.ui?.crmOutreachSelected||[]}function p(){return e.state.ui||(e.state.ui={}),Array.isArray(e.state.ui.crmOutreachDraft)||(e.state.ui.crmOutreachDraft=[...a()]),e.state.ui.crmOutreachDraft}function u(){let m=[...p()].sort((P,E)=>P-E).join(","),_=[...a()].sort((P,E)=>P-E).join(",");return m!==_}function h(){e.state.ui||(e.state.ui={}),e.state.ui.crmOutreachDraft=[],e.state.ui.crmOutreachSelected=[]}function n(){e.state.ui||(e.state.ui={}),Array.isArray(e.state.ui.crmOutreachDraft)||(e.state.ui.crmOutreachDraft=[...a()])}function t(){let m=b(),_=new Set(p()),P=a().length,E=u(),V=document.getElementById("outreach-company-picker");if(!V)return;V.querySelectorAll("[data-crm-company-toggle]").forEach(j=>{let Y=parseInt(j.dataset.crmCompanyToggle,10),D=_.has(Y);j.checked=D,j.disabled=!D&&_.size>=m,j.closest(".outreach-company-row")?.classList.toggle("is-selected",D)});let Q=V.querySelector(".outreach-select-meter__fill");Q&&(Q.style.width=`${Math.min(100,_.size/m*100)}%`);let K=document.getElementById("outreach-draft-count");K&&(K.textContent=String(_.size));let o=document.getElementById("outreach-batch-max");o&&(o.textContent=` / ${m}`);let d=document.getElementById("outreach-select-meter");d&&(d.setAttribute("aria-valuenow",String(_.size)),d.setAttribute("aria-valuemax",String(m)));let f=document.getElementById("outreach-saved-count");f&&(f.textContent=String(P));let $=document.getElementById("outreach-selection-dirty");$&&($.hidden=!E);let R=document.getElementById("outreach-save-companies");R&&(R.disabled=!E||_.size===0,R.classList.toggle("is-pulse",E&&_.size>0));let G=document.getElementById("outreach-start-btn");if(G){let j=S(),Y=P>0&&j!=="root"&&!E;G.disabled=!Y,E?G.title="Save your company selection before starting":P?G.title="":G.title="Select and save at least one company"}let O=document.getElementById("outreach-batch-hint");O&&(O.textContent=_.size>=m?`Batch limit reached (${m})`:`Up to ${m} companies per campaign`)}function s(m){let _=parseInt(m.dataset.crmCompanyToggle,10);if(!_)return;e.state.ui||(e.state.ui={});let P=b(),E=new Set(p());if(m.checked){if(E.size>=P){m.checked=!1;return}E.add(_)}else E.delete(_);e.state.ui.crmOutreachDraft=[...E],t()}function l(){e.state.ui||(e.state.ui={});let m=p();if(!m.length)return;e.state.ui.crmOutreachSelected=[...m];let _=S();if(_)try{localStorage.setItem(`fos_outreach_sel_${_}`,JSON.stringify(m))}catch{}t();let P=document.getElementById("outreach-save-companies");P&&(P.classList.add("is-saved-flash"),setTimeout(()=>P?.classList.remove("is-saved-flash"),600))}function i(m){e.state.ui||(e.state.ui={});let _=parseInt(m,10)||5;e.state.ui.crmOutreachBatch=_;let P=p();P.length>_&&(e.state.ui.crmOutreachDraft=P.slice(0,_)),t()}function g(m){let _=(m||"").trim().toLowerCase();document.querySelectorAll("#outreach-company-picker .outreach-company-row").forEach(P=>{let E=(P.dataset.search||"").toLowerCase();P.hidden=!!(_&&!E.includes(_))})}function T(){let m=S();return(e.state._crmCompanies?.companies||[]).filter(_=>m&&m!=="root"&&_.world_id&&_.world_id!==m?!1:_.status==="prospect"||!_.status)}function A(m){if(m.channel==="email"){if(!(m.subject||"").trim())return"Subject required";if(!(m.body||"").trim())return"Body required";if(!(m.email||"").trim())return"Contact has no email"}if(m.channel==="whatsapp"){if(!(m.body||"").trim())return"Message required";if((m.body||"").length>300)return"Max 300 characters";if(!m.whatsapp_enabled)return"WhatsApp not allowlisted";if(!(m.phone||"").trim())return"No phone on contact"}return""}function v(m){let _=[["setup","1. Setup"],["running","2. Research & draft"],["review","3. Review & send"],["complete","4. Done"]],E={setup:0,running:1,review:2,complete:3}[m]??0;return`<nav class="crm-outreach-steps" aria-label="Outreach progress">${_.map(([V,Q],K)=>`<span class="${K<E?"crm-outreach-step crm-outreach-step--done":K===E?"crm-outreach-step crm-outreach-step--active":"crm-outreach-step"}">${e.esc(Q)}</span>`).join("")}</nav>`}function y(){let m=e.state._crmOutreachJob||{},_=e.state._crmCampaignDetail?.campaign||e.state._crmCampaignReview?.campaign||{},P=m.phase||_.status||"Starting\u2026",V=(e.state._crmCampaignReview?.companies||e.state._crmCampaignDetail?.review?.companies||[]).length||_.batch_size||"?";return`<section class="driver-card span-12 crm-outreach-running">
      <p class="section-eyebrow">Outreach in progress</p>
      <h3 class="title-sm">${e.esc(_.name||"Campaign")}</h3>
    ...
    </section>`}function L(m){let _=m.progress||{},P=_.by_status||{};return`<section class="driver-card span-12">
      <p class="section-eyebrow">Campaign complete</p>
      <h3 class="title-sm">${e.esc(m.campaign?.name||"Campaign")}</h3>
      ...
      <div class="crm-outreach-summary">
        ...
        <div class="kv"><span class="k">Companies</span><span class="v">${_.companies_complete||0}/${_.companies_total||0}</span></div>
      </div>
      <div class="human-form__actions" style="margin-top:var(--space-md)">
    ...
    </section>`}function W(m){let _=m.campaign,P=m.strategy||{},E=m.current_company,V=m.current_research||{},Q=m.current_drafts||[],K=m.progress||{},o=Q.filter(O=>O.channel==="email"),d=Q.filter(O=>O.channel==="whatsapp"),f=E?.company_name||E?.name||"Company",$=K.company_index||1,R=K.companies_total||1,G=O=>{let j=e.draftApproveDisabledReason(O),Y=(O.body||"").length;return`<div class="crm-draft-card driver-card" data-draft-id="${O.id}">
        ...
        <label class="human-field"><span class="caption-uppercase">Message</span>
          ...
          ${O.channel==="whatsapp"?`<span class="caption muted crm-wa-count" data-draft-id="${O.id}">${Y}/300</span>`:""}
        </label>
        <div class="human-form__actions">
          <button type="button" class="button-primary button-sm" data-crm-draft-approve="${O.id}" ${j?'disabled title="'+e.esc(j)+'"':""}>Approve &amp; Send</button>
        ...
        ${j?`<p class="muted body-sm">${e.esc(j)}</p>`:""}
      </div>`};return`<section class="driver-card span-12">
      <div class="human-panel__head">
        <div>
          <p class="section-eyebrow">Review &amp; send</p>
          <h3 class="title-sm">${e.esc(_.name||"Campaign")}</h3>
          <p class="muted body-sm">Company ${$} of ${R} \xB7 ${m.pending_count||0} message(s) left \u2014 approve one at a time</p>
        </div>
      ...
      <div class="crm-outreach-progress-meta">
        ...
        <div class="crm-outreach-stats">
          ...
          <span class="badge-pill">Pending ${m.pending_count||0}</span>
        </div>
      </div>
      ...
      ${E?`<div class="crm-company-review driver-card">
        <div class="human-panel__head">
          <h4 class="title-sm">${e.esc(f)}</h4>
          <button type="button" class="button-outline-on-dark button-sm" data-crm-skip-company="${E.company_id}">Skip company</button>
        </div>
        <p class="body-sm muted">${e.esc(V.sector||E.sector||"")}</p>
        ${V.crm_research_summary?`<p class="body-sm">${e.esc(String(V.crm_research_summary).slice(0,400))}</p>`:""}
        ${(V.web_hits||[]).length?`<p class="caption-uppercase">Web signals</p><ul class="list-plain">${V.web_hits.slice(0,3).map(O=>`<li class="body-sm">${e.esc(O.snippet||O.title||"")}${O.url?` <a href="${e.esc(O.url)}" target="_blank" rel="noopener">link</a>`:""}</li>`).join("")}</ul>`:""}
        ${(V.vault_files_used||[]).length?`<p class="caption-uppercase">Vault files used</p><ul class="list-plain">${V.vault_files_used.map(O=>`<li class="body-sm">${e.esc(O.title||"doc #"+O.doc_id)}</li>`).join("")}</ul>`:""}
      </div>`:""}
      ${o.length?'<p class="caption-uppercase">Email drafts</p>':""}
      ${o.map(G).join("")}
      ${d.length?'<p class="caption-uppercase" style="margin-top:var(--space-md)">WhatsApp drafts</p>':""}
      ${d.map(G).join("")}
      ${!Q.length&&E?'<p class="muted">No drafts for this company \u2014 contacts may lack email or WhatsApp allowlist.</p>':""}
    </section>`}function N(m){e.state.ui||(e.state.ui={});let _=[];if(m)try{let P=localStorage.getItem(`fos_outreach_sel_${m}`),E=P?JSON.parse(P):[];_=Array.isArray(E)?E.filter(V=>Number.isFinite(V)):[]}catch{}e.state.ui.crmOutreachSelected=_,e.state.ui.crmOutreachDraft=[..._]}function q(){n();let m=e.state._crmCampaigns?.campaigns||[],_=S(),P=T(),E=b(),V=new Set(p()),Q=a().length,K=u(),d=((e.state.worlds||e.state._worldFull?.worlds||{}).children||[]).length>0,f=e.state._crmCompaniesLoading,$=e.state._crmCompaniesError,R=P.map(D=>{let X=V.has(D.id),B=D.contact_count||0,te=`${D.name||""} ${D.sector||""}`.trim();return`<label class="outreach-company-row human-field--checkbox${X?" is-selected":""}" data-search="${e.esc(te)}">
        <input type="checkbox" data-crm-company-toggle="${D.id}" ${X?"checked":""} ${V.size>=E&&!X?"disabled":""}>
        <span class="outreach-company-row__main">
          <span class="outreach-company-row__name">${e.esc(D.name)}</span>
          <span class="outreach-company-row__meta muted">${e.esc(D.sector||"\u2014")} \xB7 ${B} contact${B===1?"":"s"}</span>
        </span>
      </label>`}).join(""),G=[5,10,15,20].map(D=>`<option value="${D}"${E===D?" selected":""}>${D}</option>`).join(""),O=m.slice(0,12).map(D=>`<tr>
        <td><button type="button" class="${D.status==="review"?"button-primary":"button-tertiary-text"} button-sm" data-crm-campaign="${D.id}">${e.esc(D.name)}</button></td>
        <td><span class="badge-pill badge-pill--${e.esc(D.status)}">${e.esc(D.status)}</span></td>
        <td class="muted">${e.esc((D.created_at||"").slice(0,10))}</td>
        <td>${D.status==="review"?`<button type="button" class="button-outline-on-dark button-sm" data-crm-campaign="${D.id}">Continue review</button>`:""}</td>
      </tr>`).join("")||'<tr><td colspan="4" class="muted">No campaigns yet</td></tr>',j=P.length?`<div id="outreach-company-picker" class="outreach-company-picker">
          <div class="outreach-picker-toolbar">
            <div class="outreach-picker-toolbar__head">
              ...
              <div class="outreach-picker-toolbar__counts">
                <span class="outreach-count-pill" title="Currently selected (not yet saved)">
                  <strong id="outreach-draft-count">${V.size}</strong><span class="muted" id="outreach-batch-max"> / ${E}</span>
                </span>
                <span class="outreach-count-pill outreach-count-pill--saved" title="Saved for this campaign">
                  <strong id="outreach-saved-count">${Q}</strong> saved
                </span>
                <span id="outreach-selection-dirty" class="outreach-dirty-badge"${K?"":" hidden"}>Unsaved</span>
              </div>
            </div>
            <div class="outreach-select-meter" id="outreach-select-meter" role="progressbar" aria-valuenow="${V.size}" aria-valuemin="0" aria-valuemax="${E}" aria-label="Selection progress">
              <div class="outreach-select-meter__fill" style="width:${Math.min(100,V.size/E*100)}%"></div>
            </div>
            <p class="body-sm muted" id="outreach-batch-hint">${V.size>=E?`Batch limit reached (${E})`:`Pick up to ${E}, then save`}</p>
            <div class="outreach-picker-toolbar__actions">
              ...
              <button type="button" id="outreach-save-companies" class="button-outline-on-dark button-sm" data-outreach-save-companies ${K&&V.size?"":"disabled"}>Save selection</button>
            </div>
          </div>
        ...
        </div>`,Y=Q>0&&_!=="root"&&!K;return`<section class="driver-card span-12 human-panel outreach-setup">
      <div class="human-panel__head">
        <div>
      ...
      ${d?"":'<p class="crm-outreach-warn">Create a sub-world under <strong>World</strong> first \u2014 outreach needs a venture context for vault research.</p>'}
      ${$?`<p class="crm-draft-error">${e.esc($)}</p>`:""}
      <form class="human-form outreach-setup-form" id="crm-outreach-form">
        <div class="outreach-setup-grid">
          <label class="human-field"><span class="caption-uppercase">World</span>
            <select class="text-input-on-dark" name="world_id" id="crm-outreach-world">${e.renderWorldOptionsForCrm(_)}</select></label>
        ...
        ${f?'<p class="muted body-sm">Loading companies\u2026</p>':j}
        <div class="human-form__actions outreach-setup-actions">
          <button type="submit" id="outreach-start-btn" class="button-primary" ${Y?"":"disabled"}${K?' title="Save your company selection before starting"':Q?"":' title="Select and save at least one company"'}>
            Start outreach${Q?` (${Q} companies)`:""}
          </button>
        </div>
      </form>
    ...
    </section>`}function z(){let m=e.outreachStep(),_=e.state._crmCampaignReview;return m==="running"?e.renderOutreachRunningPanel():m==="complete"&&_?.campaign?e.renderOutreachCompletePanel(_):m==="review"&&_?.campaign?e.renderOutreachReviewPanel(_):e.renderOutreachSetupPanel()}function Z(){return`<div class="dashboard-grid">
      <section class="driver-card span-12 crm-shell">
        <div class="human-panel__head crm-shell__head">
          <div>
    ...
    </div>`}async function ae(){e.state.ui||(e.state.ui={}),e.state.ui.crmOutreachWorld||(e.state.ui.crmOutreachWorld=e.currentWorldId());let m=e.outreachWorldId(),_=m&&m!=="root"?`?world_id=${encodeURIComponent(m)}&include_unassigned=1`:"?include_unassigned=1",P=m&&m!=="root"?`?world_id=${encodeURIComponent(m)}`:"",E=e.routeParams?.campaignId||e.state.ui?.crmCampaignId;e.state._crmCompaniesLoading=!0,e.state._crmCompaniesError=null;try{let[V,Q]=await Promise.all([e.api(`/crm/companies${_}`),e.api(`/crm/outreach/campaigns${P}`).catch(()=>({campaigns:[]}))]);if(e.state._crmCompanies=V,e.state._crmCampaigns=Q,E||Array.isArray(e.state.ui.crmOutreachDraft)||(a().length?e.state.ui.crmOutreachDraft=[...a()]:N(m)),E){e.state.ui.crmCampaignId=E;let[K,o]=await Promise.all([e.api(`/crm/outreach/campaigns/${E}`).catch(()=>null),e.api(`/crm/outreach/campaigns/${E}/review`).catch(()=>null)]);e.state._crmCampaignDetail=K,e.state._crmCampaignReview=o?.campaign?o:K?.review;let d=e.state._crmCampaignReview?.campaign||K?.campaign;d&&["researching","drafting","created"].includes(d.status)?(e.state._crmOutreachJob={active:!0,phase:d.status,status:d.status},e.state._crmOutreachPollId||e.pollCrmOutreachJob(E)):d?.status==="review"&&(e.state._crmOutreachJob={phase:"Ready for review",active:!1})}}catch(V){e.state._crmCompaniesError=V.message||"Could not load outreach data"}finally{e.state._crmCompaniesLoading=!1}}async function J(m){let _=new FormData(m),P=(_.get("world_id")||"").toString().trim(),E=parseInt(_.get("batch_size")||"5",10)||5,V=(_.get("brief")||"").toString().trim(),Q=a();if(u())return alert("Save your company selection before starting.");if(!P||P==="root")return alert("Select a sub-world for outreach (not Main world).");if(!Q.length)return alert("Select and save at least one company.");if(!V)return alert("Add a brief so the agent knows what kind of message to write.");try{let K=await e.api("/crm/outreach/campaigns",{method:"POST",body:JSON.stringify({world_id:P,batch_size:E,brief:V,company_ids:Q})});await e.api(`/crm/outreach/campaigns/${K.campaign_id}/start`,{method:"POST"}),e.state.ui||(e.state.ui={}),e.state.ui.crmOutreachSelected=[],e.state.ui.crmOutreachDraft=[];try{localStorage.removeItem(`fos_outreach_sel_${P}`)}catch{}e.goView("outreach",{params:{campaignId:K.campaign_id}}),e.pollCrmOutreachJob(K.campaign_id)}catch(K){alert(K.message)}}async function C(m,_=!1){e.state._crmOutreachPollId&&clearTimeout(e.state._crmOutreachPollId);let P=async()=>{try{let E=await e.api(`/crm/outreach/campaigns/${m}`),V=E.campaign||{},Q=E.review||{},K=E.job||{};if(e.state._crmCampaignDetail=E,V.status==="review"||V.status==="done"||V.status==="failed"){e.state._crmOutreachJob={active:!1,phase:V.status==="review"?"Ready for review":V.status},e.state._crmCampaignReview=Q.campaign?Q:await e.api(`/crm/outreach/campaigns/${m}/review`),e.state._crmOutreachPollId=null,e.currentView==="outreach"&&e.render();return}e.state._crmOutreachJob={active:!0,phase:K.phase||V.status||"running\u2026",status:V.status},e.currentView==="outreach"&&e.render(),_||(e.state._crmOutreachPollId=setTimeout(P,2500))}catch{_||(e.state._crmOutreachPollId=setTimeout(P,4e3))}};_?await P():e.state._crmOutreachPollId=setTimeout(P,500)}async function r(m){m&&e.goView("outreach",{params:{campaignId:parseInt(m,10)}})}function c(){e.state._crmOutreachPollId&&clearTimeout(e.state._crmOutreachPollId),e.state._crmOutreachPollId=null,e.state.ui&&(e.state.ui.crmCampaignId=null),e.state._crmCampaignReview=null,e.state._crmCampaignDetail=null,e.state._crmOutreachJob=null,e.goView("outreach",{params:{}})}function w(m){s(m)}async function I(m){let _=e.state.ui?.crmCampaignId;if(!(!_||!m)&&confirm("Skip all pending messages for this company?"))try{await e.api(`/crm/outreach/campaigns/${_}/companies/${m}/skip`,{method:"POST"}),e.state._crmCampaignReview=await e.api(`/crm/outreach/campaigns/${_}/review`),e.render()}catch(P){alert(P.message)}}async function M(m){let _=document.querySelector(`.crm-draft-subject[data-draft-id="${m}"]`),P=document.querySelector(`.crm-draft-body[data-draft-id="${m}"]`),E={};_&&(E.subject=_.value),P&&(E.body=P.value),Object.keys(E).length&&await e.api(`/crm/outreach/drafts/${m}`,{method:"PATCH",body:JSON.stringify(E)})}async function F(m){if(m)try{await e.saveCrmDraftEdits(m);let _=await e.api(`/crm/outreach/drafts/${m}/approve-send`,{method:"POST"});if(_.error)return alert(_.error);let P=e.state.ui?.crmCampaignId;P&&(e.state._crmCampaignReview=await e.api(`/crm/outreach/campaigns/${P}/review`)),e.render()}catch(_){alert(_.message)}}async function U(m){if(m)try{await e.api(`/crm/outreach/drafts/${m}/skip`,{method:"POST"});let _=e.state.ui?.crmCampaignId;_&&(e.state._crmCampaignReview=await e.api(`/crm/outreach/campaigns/${_}/review`)),e.render()}catch(_){alert(_.message)}}e.outreachWorldId=S,e.outreachStep=k,e.draftApproveDisabledReason=A,e.renderOutreachSteps=v,e.renderOutreachRunningPanel=y,e.renderOutreachCompletePanel=L,e.renderOutreachReviewPanel=W,e.renderOutreachSetupPanel=q,e.renderOutreachBody=z,e.renderOutreach=Z,e.loadOutreachData=ae,e.submitCrmOutreach=J,e.pollCrmOutreachJob=C,e.openCrmCampaignReview=r,e.closeCrmCampaignReview=c,e.toggleOutreachDraftCompany=s,e.saveOutreachCompanySelection=l,e.setOutreachBatchSize=i,e.filterOutreachCompanyList=g,e.syncOutreachCompanyPickerUi=t,e.restoreOutreachSelectionForWorld=N,e.resetOutreachCompanySelection=h,e.toggleCrmOutreachCompany=w,e.skipCrmCompany=I,e.saveCrmDraftEdits=M,e.approveCrmDraft=F,e.skipCrmDraft=U}function Ie(e){function S(){let u=e.state._goals||{},h=!!e.state.ui?.goalsFormOpen,n=!!e.state.ui?.reminderFormOpen,t=(u.active||[]).map(g=>`<li class="goal-row">
      ...
      </div>`}async function k(){let b=e.$("#memory-q")?.value?.trim();if(e.state._memoryQ=b,!b)return;let a=await e.api("/memory/search?q="+encodeURIComponent(b));e.state._memoryResults=a.results,e.render()}e.renderMemory=S,e.searchMemory=k}function Le(e){function S(a){let p=a.content||"";return a.role==="agent"||a.role==="assistant"?`<div class="msg-md history-msg__body">${window.FOSMarkdown?.render?.(p)||e.esc(p)}</div>`:`<p class="body-md history-msg__body">${e.esc(p)}</p>`}function k(){let p=(e.state._history||{}).sessions||[],u=e.state._artifacts||[],h=e.state._historySession,n=e.historyTab,t=p.length?p.map(i=>`
      <button type="button" class="history-session${h?.id===i.id?" is-active":""}" data-history-session="${e.esc(i.id)}">
      ...
      </button>`).join(""):"<p class='body-md muted'>No conversations yet. Ask the agent something to start a session.</p>",s="<p class='body-md muted'>Select a conversation to view messages, runs, and linked documents.</p>";if(h?.messages?.length){let i=h.messages.map(A=>`
        ...
        </article>`).join("")||"",T=(h.artifacts||[]).map(A=>`
        ...
        <div class="history-artifacts">${T}</div>`}let l=u.length?u.map(i=>`
    ...
    <div class="tool-list">${b}</div>`}e.renderTools=S}function De(e){function S(){let k=e.state._activity?.traces_full||[],b=e.state._activity?.actions||e.state.actions||[],a=k.length?k.map(u=>`
      ...
      </section>`;let t=n.host||{},s=n.s3||{},l=n.disk||{},i=n.app||{},g=t.platform==="ec2"?e.infraKvRow("Instance",t.instance_id,!0)+e.infraKvRow("Region",t.region)+e.infraKvRow("Type",t.instance_type)+e.infraKvRow("IAM role",t.iam_role):e.infraKvRow("Host","Local / dev"),T=s.configured?e.infraKvRow("Bucket",s.bucket,!0)+e.infraKvRow("Region",s.region)+e.infraKvRow("Read/write",s.read_write_ok?"OK":s.reachable?"Reachable only":"Failed"):e.infraKvRow("Storage","Local disk only"),A=e.infraKvRow("Data path",l.path,!0)+e.infraKvRow("Free",l.free_gb!=null?`${l.free_gb} GB`:null)+e.infraKvRow("Used",l.used_pct!=null?`${l.used_pct}%`:null),v=!!n.ok;return`<section class="driver-card span-12">
      <div class="infra-health-head">
        <div>
          <p class="caption-uppercase">Infrastructure</p>
